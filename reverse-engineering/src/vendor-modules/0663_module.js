...
function (e, t, i) {
      ...
      (r.inheritAndMix(u, r, [h]),
        ...
        (u.prototype._numHorizontal = 0),
        (u.prototype._numVertical = 0),
        (u.prototype._consolidatedCanvas = null),
        (u.prototype._renderQueue = null),
        (u.prototype._iterateGrid = function (e, t) {
          for (var i, n = 0, r = 0, o = 0; o < this._numHorizontal; o++) {
            r = 0;
            for (var a = 0; a < this._numVertical; a++) {
              if (
                ((i = this._paintCanvases[o * this._numVertical + a]),
                !t || (t && !i.$renderedBitmap))
              ) {
                var s = e(i, n, r);
                if (!1 === s) return s;
              }
        ...
        (u.prototype.resize = function (e, t) {
          if (e != this.getWidth() || t != this.getHeight()) {
            for (
              var i,
                n,
                r = e / this._numHorizontal,
                o = t / this._numVertical,
                a = Math.floor(r),
                s = Math.floor(o),
                l = e - a * (this._numHorizontal - 1),
                h = t - s * (this._numVertical - 1),
                A = 1,
                c = 1,
                p = 0;
              p < this._numHorizontal;
              p++
            ) {
              ((i = p === this._numHorizontal - 1 ? l : a),
                (A = Math.max(A, i)));
              for (var u = 0; u < this._numVertical; u++)
                ((n = u === this._numVertical - 1 ? h : s),
                  (c = Math.max(c, n)),
                  this._paintCanvases[p * this._numVertical + u].resize(i, n));
            }
            (this._panHelper.resize(A, c),
              this._paintCanvasCache.resize(e, t),
        ...
        (u.prototype._renderNext = function () {
          return 1 === this._renderQueue.length
            ? ((this._renderQueue = []), !1)
            : ((this._renderQueue = [
                this._renderQueue[this._renderQueue.length - 1],
              ]),
              this._scheduleRender(),
              !0);
        }),
        (u.prototype._scheduleRender = function () {
          if (
            !(0 === this._renderQueue.length || this._renderQueue.length > 1)
          ) {
            ...
            this._paintCanvases.some(function (e) {
              ...
              ? console.log(
                  "SCHEDULERENDER: ERROR: render phase render, waiting",
                )
        ...
        (u.prototype._paintAndRenderInternal = function (
          e,
          t,
          i,
          n,
          ...
          var u,
            ...
            B = function (e, i, n, o) {
              ...
              if ((n && (v = !0), f === g))
                ...
                (A.DRAW_DURING_RENDERING || A.SHOW_LOWRES_GIMMICK) &&
                (i || 1 === g)
              )
            ...
            P = [];
          ((d =
            c || 1 === m._paintCanvases.length || m.isThreaded()
              ? m._iterateGrid
              : m._iterateGridAsync).call(m, function (c, u, d) {
            var f = new a(u, d, c.getWidth(), c.getHeight()),
              v = _.intersect(f);
            if (v) {
              var S;
              g++;
              ...
              if (
                ((S = A.ENABLE_CACHE
                  ? m._paintCanvasCache.cachedRender(c, v, n, r, u, d)
                  : v),
                A.SHOW_LOWRES_GIMMICK_FOR_QUICKRENDER &&
                  ((l.getRenderParameters() || {}).quickRender ||
                    (l.getRenderParameters() || {}).noWebGL) &&
                  S &&
                  1 === m._paintCanvases.length)
              ) {
                var I = S.getDirtyRectangles().slice();
                ((E[x] = !0),
                  (T = m._paintCanvasCache.getGimmick(I, n, r)) && (S = null));
              }
              if (S) {
                ((v = S).getDirtyRectangles() ||
                  console.warn("EMPTY DIRTY RECTS"),
                  v.transform(new s().translated(-u, -d)),
                  c.prepare(v.getDirtyRectangles()),
                  (b[x] = v.getDirtyRectangles().slice()),
                  (C[x] = n.add(new o(u, d))),
                  v.transform(new s().translated(u, d)),
                  c.setOrigin(C[x]),
                  c.setScale(r),
              ...
              try {
                A.DEBUG_GRID_PAINT_DELAY > 0
                  ...
                  : ((w[x] = !!S && Date.now()),
                    T
                      ? B(T._canvasContext.canvas, !0, !1)
                      : l.render(c.getRendererContext(), 0, B));
              } catch (e) {
                return (
        ...
        (u.prototype._getGridIndexForNativeCanvas = function (e) {
          for (var t = 0; t < this._paintCanvases.length; t++)
            if (this._paintCanvases[t].getRendererContext()) {
              if (
                this._paintCanvases[t].getRendererContext().$realCtx.canvas ===
                e
              )
                return t;
            } else if (e instanceof n) {
              if (self._paintCanvases[t] === e) return t;
        ...
        (u.prototype.moveContent = function (e, t) {
          ...
          (i.resize(this.getWidth(), this.getHeight()),
            i.clearRect(0, 0, i.getWidth(), i.getHeight()),
            this.drawFragmentTo(
              this._panHelper,
              e.getX(),
              e.getY(),
          ...
          this._iterateGrid(function (e, t, i) {
            var l = new a(t, i, e.getWidth(), e.getHeight()),
              h = n.intersected(l),
              A = e.getRendererContext().$realCtx;
            (A
              ? A.clearRect(0, 0, e.getWidth(), e.getHeight())
              : e.clearRect(0, 0, e.getWidth(), e.getHeight()),
              !h.isEmpty() &&
                A &&
                A.drawImage(
                  s._panHelper._canvasContext.canvas,
                  h.getX() + r,
                  h.getY() + o,
                  h.getWidth(),
                  h.getHeight(),
        ...
        (u.prototype.drawFragmentTo = function (e, t, i, n, r, o, s) {
          ...
          this._iterateGrid(function (t, i, o) {
            var s = new a(i, o, t.getWidth(), t.getHeight()),
              h = l.intersected(s);
            h.isEmpty() ||
              e.drawImageFragment(
                t,
        ...
        (u.prototype.getSubCanvasContainingGraphic = function (e) {
          var t = null;
          return (
            this._iterateGrid(function (i, n, r) {
        ...
        (u.prototype.drawTo = function (e, t, i) {
          ...
          if (t) {
            ...
            if (
              ((n = this._iterateGrid(function (t, n, o) {
                if (i) {
                ...
                for (
                  var l = new a(n, o, t.getWidth(), t.getHeight()), h = 0;
                  h < r.length;
                  ++h
                ) {
              ...
              for (
                var o = new a(0, 0, i.width, i.height), s = 0;
                s < r.length;
                ++s
              ) {
                var l = r[s].toAlignedRect();
                (l = l.intersected(o)).isEmpty() ||
                  e.drawImageFragment(
                    i,
                    l.getX(),
                    l.getY(),
        ...
        (u.prototype.destroy = function () {
          ...
          (this._panHelper.destroy(),
            this._consolidatedCanvas &&
              (this._consolidatedCanvas.destroy(),
              (this._consolidatedCanvas = null)),
            this.cleanCache());
        }),
