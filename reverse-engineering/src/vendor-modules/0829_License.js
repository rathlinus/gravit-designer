      ...
      class A {
        ...
        isOfflinePeriodExpired(e) {
          return (
            this.isOffline() &&
            !!this._offlineExpire &&
            h.lte(this._offlineExpire, e || h.now())
          );
        }
        ...
        getOfflineWarningDate() {
          return this.getOfflineExpirationDate()
            ? h.addTime(
                this.getOfflineExpirationDate(),
                -a + h.daysToMilliseconds(1),
              )
            : null;
        }
        getQuotas() {
        ...
        isProSubscriptionCancelled() {
          return (
            !!this.isPro() &&
            !!this.isProSubscriptionCancellable() &&
        ...
        getIntercomUserType() {
          return (
            this._metadata &&
            this._metadata.intercom &&
            this._metadata.intercom.userType
          );
        }
        ...
        daysLeft() {
          return this._expire
            ? h.millisecondsToDays(h.diff(h.now(), this._expire))
            : null;
        }
        toJSON() {
          return {
