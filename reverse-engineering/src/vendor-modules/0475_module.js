...
function (e, t, i) {
      ...
      function a(e) {
        (n.call(
          ...
          e,
          null,
          "        varying highp vec2 texCoord;        uniform highp sampler2D texture0;        uniform highp sampler2D texture1;        uniform float bloom_intensity;        uniform float base_intensity;        uniform float bloom_saturation;        uniform float base_saturation;        vec4 adjustSaturation(vec4 color, float saturation){            float grey = dot(color.rgb, vec3(0.3, 0.59, 0.11));            return vec4(mix(grey, color.r, saturation),mix(grey, color.g, saturation),mix(grey, color.b, saturation),color.a);        }        void main(){            vec4 bloom = texture2D(texture1, texCoord);            vec4 base = texture2D(texture0, texCoord);            bloom = adjustSaturation(bloom, bloom_saturation);            bloom.rgb *= bloom_intensity;            base = adjustSaturation(base, base_saturation);            base.rgb *= base_intensity;            base *= (1.0 - clamp(bloom,0.0,1.0));            gl_FragColor = base + bloom;        }    ",
        ),
          this.textures({
            texture1: 1,
          }),
          (this.brightness = new n(
            e,
            null,
            "        uniform highp sampler2D texture;        varying highp vec2 texCoord;        uniform float bloom_threshold;        uniform float bloom_ithreshold;        void main(){            vec4 c = texture2D(texture, texCoord);            gl_FragColor = vec4(c.rgb*bloom_ithreshold - bloom_threshold,c.a);        }        ",
          )),
          (this.blur = new r(e)));
      }
      ((a.prototype.brightness = null),
        ...
        (a.prototype.destroy = function () {
          (this.brightness && this.brightness.destroy(),
            this.blur && this.blur.destroy(),
