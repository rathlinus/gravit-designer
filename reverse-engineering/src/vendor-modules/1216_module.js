...
function (e, t, i) {
      ...
      function Z(e, t) {
        ((this._doc = e),
          (this._graphics = t || e.createGraphics()),
          (this._saveRestoreStack = []),
          (this._pageSize = this._doc.getPageSize()),
          (this._pageOrigin = this._doc.getPageOrigin()),
          (this._resources = e.getCurrentResources()));
      }
      ((Z.prototype._resources = null),
        (Z.prototype._pageSize = null),
        (Z.prototype._pageOrigin = null),
        (Z.prototype._bitmapCache = null),
        (Z.prototype.createGraphics = function () {
        ...
        (Z.prototype._makeFillStyle = function () {
          if (
            this.canvas.getCurrentNode() instanceof S &&
            this.canvas._paintContext._borderPaintLayer
          ) {
            var e;
            do {
              if ((e = this._graphics.peek().getValue()) instanceof C) break;
              this._graphics.pop();
            } while (e);
        ...
        (Z.prototype.getParent = function () {
          return this.canvas._parent
            ? this.canvas._parent._canvasContext
            : null;
        }),
        ...
        (Z.prototype.accept = function (e) {
          e(this) ||
            (this.canvas._children &&
              a.each(this.canvas._children, function (t, i) {
        ...
        (Z.prototype.drawImage = function (e, t, i, n, r, o, a, s, l) {
          ...
          if (h instanceof Y) {
            var A = h.getType() && h.getType() === Y.Type.JPEG,
              c = h.getImageCanvas(),
              p = c.width,
              u = c.height;
            ((o = o || t || 0),
            ...
            if (this._doc.isDownsamplingImages()) {
              var g = W.getMaximumCanvasSize(),
                f = this._transform.mapRect(new P(0, 0, s, l));
              if (
                !f.isEmpty() &&
                f.getWidth() < g.width &&
                f.getHeight() < g.height &&
                f.getWidth() * f.getHeight() < g.area
              ) {
                var m = h.getProperty("iw"),
                  y = h.getProperty("ih");
                if (m && y && f.getWidth() < m && f.getHeight() < y) {
                  var _ = Math.min(
                      f.getWidth() / c.width,
                      f.getHeight() / c.height,
                    ),
                    b = document.createElement("canvas");
                  ((b.width = Math.ceil(c.width * _)),
                    (b.height = Math.ceil(c.height * _)),
                    b
                      .getContext("2d")
                      .drawImage(
                        c,
            ...
            (this._hasBlendMode() || 1 != this.globalAlpha) &&
              this._graphics.setGStateResource(
                this._createGStateResource(
                  this.globalAlpha,
                  v.OPERATIONFLAG_FILL,
                  this.globalCompositeOperation,
                ),
              );
            var w = new v(this._doc);
            (w.add(new Q(new T(p, 0, 0, u, 0, -u))),
              w.add("/" + C.getName() + " Do"),
              this._graphics.add(new Q(this._doc.convertToPDFCoordinates(d))),
              this._graphics.add(w));
          }
        }),
        (Z.prototype.replaceWithBitmap = function (e, t, i, n, r) {
          ...
          if (
            ...
            (t instanceof Y && (a = t.isJPEG()),
              ...
              this._bitmapCache.push({
                bitmap: e,
                resource: o,
              }));
          }
          var s = (i = i || t.getPaintBBox()).getX(),
            l = i.getY(),
            h = new P(s, l, e.width, e.height),
            A = 1 / this.canvas.getScale();
          h = (h = h.scaled(A, A))
          ...
          var c = new T(
              ...
              h.getHeight(),
              h.getX(),
              this._y(h.getY()) - h.getHeight(),
            ),
            p = new d(t, d.Type.BEGIN),
            u = new d(t, d.Type.END);
          this.accept(
            function (e) {
              e._graphics.accept(
                function (i, a) {
                  if (i.equals(p)) {
                    ...
                    if (l && l.getValue() instanceof v)
                      (d = l.getValue().getGSStateResource()) &&
                    ...
                    if (r && !s.getGSStateResource() && t.hasMixin(m)) {
                      ...
                      if (this._hasBlendMode(h) || 1 != A) {
                        var d = this._createGStateResource(
                          A,
                          v.OPERATIONFLAG_FILL,
                          h,
                        );
        ...
        (Z.prototype._createImageResource = function (e, t, i) {
          ...
          var s = function () {
              return (
                this._doc.hasConcurrencyFeature() &&
                e.width > 100 &&
                e.height > 100
              );
            }.bind(this),
            l = this._resources || this._doc.getCurrentResources(),
            h = function (e, i) {
              if (i) {
                var r = this._doc.getIndirectObject(o);
                (o.setFilter(new U(i)),
                  a.getPDFObject().setMask(new R(r)),
                  this._doc.addIndirectObject(r),
                  l.add(c.Group.Types.XOBJECT, new R(r)));
              }
              if (s())
                if (t) {
                  var h = this._doc.beginJob("Encode JPEG"),
                    A = n === I.CMYK ? O.ColorSpace.CMYK : O.ColorSpace.RGBA;
                  new O()
                    .concurrenyEncode(
                      this._doc,
                      this._doc.getWorkers(),
                      e.buffer,
                      a.getPDFObject().getWidth(),
                      a.getPDFObject().getHeight(),
                      A,
                      this._doc.getJPEGQuality(),
                    )
                    .then(
                      function (e) {
                        var t = new V();
                        (t.setBuffer(new Uint8Array(e)),
                          a.getPDFObject().setFilter(t),
                          h.done());
                      }.bind(this),
                    )
                    .catch(function (e) {
          ...
          if (s()) {
            ...
            if (i) {
              ...
              var g = i
                .getContext("2d")
                .getImageData(0, 0, i.width, i.height).data;
              new X().raster(g.buffer, I.GRAY, d);
            } else d = new Uint8ClampedArray(e.width * e.height);
            var f = new Uint8ClampedArray(e.width * e.height * n.length),
              m = new X().raster(r.buffer, n, f, i ? null : d);
            h(f, i || m ? d : null);
          }
        ...
        (Z.prototype.drawCanvas = function (e, t, i, n, r) {
          if ((null != n && n < 1) || this._hasBlendMode(r)) {
            var o = this._createGStateResource(n, v.OPERATIONFLAG_FILL, r),
              a = new v(this._doc);
            a.add(e._canvasContext._graphics.clear());
            var s = new T()
                .translated(this._pageOrigin.getX(), this._pageOrigin.getY())
                .inverted()
                .getMatrix(),
              l = s[4],
              h = s[5],
            ...
            (A.push(l),
              A.push(-Math.abs(h)),
              A.push(Math.ceil(this._pageSize.width + Math.abs(l))),
              A.push(Math.ceil(this._pageSize.height + Math.abs(h))));
            var p = new y(a, A),
              u = this._doc.getIndirectObject(p),
              d = this._doc.addResource(c.Group.Types.XOBJECT, new R(u));
            (this._doc.addIndirectObject(u),
              e._canvasContext._graphics.setGStateResource(o),
              e._canvasContext._graphics.add("/" + d.getName() + " Do"));
          }
        }),
        (Z.prototype._createStyleSMaskGStateResource = function (e, t) {
          if (e.getPattern() instanceof N) {
            ...
            if (i.hasTransparency()) {
              var n = i.getGradient(),
                r = 1 / this.canvas.getScale(),
                o = new s();
              return (
                o.add(this._graphics.peek()),
                o.add(
                  new k({
                ...
                t & v.OPERATIONFLAG_FILL && o.add(A.f),
                t & v.OPERATIONFLAG_STROKE && o.add(A.S),
                this.createSMaskGStateResource(o, t)
              );
            }
          }
        }),
        (Z.prototype.createSMaskGStateResource = function (e, t) {
          var i = new T().translated(
              this._pageOrigin.getX(),
              this._pageOrigin.getY(),
            ),
            n = new M([0, 0]);
          ...
          var r = new G({
            ...
            l = new g({
              ...
              operation: t,
              origin: i,
              blendMode: this._hasBlendMode()
                ? this.globalCompositeOperation
                : null,
            }),
            h = this._doc.getIndirectObject(l),
            A = this._doc.addResource(c.Group.Types.GSTATE, new R(h));
          return (
            this._doc.addIndirectObject(h),
            this._doc.addIndirectObject(a),
            this._doc.addIndirectObject(s),
            new o(A)
          );
        }),
        (Z.prototype._createGStateResource = function (e, t, i) {
          var n = new g({
            ...
            r = this._doc.getIndirectObject(n),
            a = this._doc.addResource(c.Group.Types.GSTATE, new R(r));
        ...
        (Z.prototype.beginPath = function () {
          ((this._currentPath = new C(this._doc)),
            this._graphics.add(this._currentPath));
        }),
        ...
        (Z.prototype.stroke = function () {
          if (this._hasStroke()) {
            var e = this.strokeStyle,
              t = this.canvas.getTransform(),
              i = this.canvas.getCurrentNode();
            if (i instanceof S && this.canvas._paintContext._borderPaintLayer) {
              var n = i.createShapePaint(
                this,
                this.canvas._paintContext._borderPaintLayer.$_pt,
                this.canvas._paintContext._patternBBox || i.getPatternBBox(),
              );
              (n.transform &&
                (i.$trf && (n.transform = n.transform.multiplied(i.$trf)),
                (t = t.multiplied(n.transform))),
                (e = n.paint));
            }
            var r = 1 / this.canvas.getScale(),
              o = new b({
            ...
            if (o.isValid()) {
              ...
              ((a.strokeStyle = o),
                ...
                this.dashArray &&
                  ...
                  (a.lineDash = new F(this.dashArray)));
              var s = this._graphics;
              if (
                1 != this.globalAlpha ||
                this._hasBlendMode(this.globalCompositeOperation)
              ) {
                var l = this._createGStateResource(
                    this.globalAlpha,
                    v.OPERATIONFLAG_STROKE,
                    this.globalCompositeOperation,
                  ),
                  h = new v(this._doc);
              ...
              s.add(a);
            } else console.warn("Invalid stroke style");
          }
        }),
        (Z.prototype.fill = function (e) {
          if (this._isBackgroundFill())
            this._graphics.peek().getValue() instanceof C &&
              this._graphics.pop();
          else if (this._hasFill()) {
            var t = this.fillStyle;
            this._doc.getColorSpace() === I.CMYK &&
              this.canvas._paintContext._fillPaintLayer &&
              this.canvas._paintContext._fillPaintLayer.$_pt instanceof r &&
              (t = this.canvas._paintContext._fillPaintLayer.$_pt);
            var i = 1 / this.canvas.getScale(),
              n = new k({
            ...
            if (n.isValid()) {
              ...
              var a = this._graphics,
                s = this._createStyleSMaskGStateResource(
                  n,
                  v.OPERATIONFLAG_FILL,
                );
              if (
                (s &&
                  ((a = new v(this._doc)).setGStateResource(s),
                  this._graphics.add(a)),
                (1 != this.globalAlpha || this._hasBlendMode()) && !s)
              ) {
                var l = this._createGStateResource(
                  this.globalAlpha,
                  v.OPERATIONFLAG_FILL,
                ...
                ((a = new v(this._doc)).setGStateResource(l),
                  this._graphics.add(a));
              }
              a.add(o);
            } else console.warn("Invalid fill style");
          }
        }),
        ...
        (Z.prototype.fillRect = function (e, t, i, n) {
          if (this.globalCompositeOperation === p.CompositeOperator.SourceIn)
            this._graphics.peek().getValue() instanceof v.Fill &&
              this._makeFillStyle();
          else if (
            this.globalCompositeOperation == p.CompositeOperator.SourceOver
          ) {
            var r = new E(e, t, i, n);
            (r.transform(this.canvas.getTransform(!0)),
              this.canvas.putVertices(r),
              this.fill());
          }
        }),
        ...
        (Z.prototype._hasBlendMode = function (e) {
          ...
          return (
            a.each(
              Object.keys(p.BlendMode),
              function (i, n) {
                p.BlendMode[n] === (e || this.globalCompositeOperation) &&
                  (t = p.BlendMode[n]);
              }.bind(this),
            ),
            !!t && t !== p.BlendMode.Normal
          );
        }),
