      ...
      (n.inheritAndMix(f, r, [r.Map, r.DetailMap]),
        ...
        (f.prototype.map = function (e, t, i, n, r, u) {
          ...
          var R = function (e, t, i) {
              ...
              return (
                (t !== c.AnchorPoint.Type.Asymmetric &&
                  t !== c.AnchorPoint.Type.Symmetric &&
                  t !== c.AnchorPoint.Type.Mirror &&
                  t !== c.AnchorPoint.Type.Connector) ||
                  ((n = A.annotationHandles.path.node.smoothType),
                  i && (n = A.annotationHandles.path.node.autoType)),
                {
                  pt: e,
                  annot: n,
            ...
            D = function (e) {
              ...
              if (
                (x && (h = h.intersected(x)),
                (i = a ? a.findPivots(h) : e.findPivots(!1, h)))
              )
                for (t = 0; t < i.length; ++t) {
                  var A = (B = i[t]).point ? B.point : B;
                  if (r || h.containsPoint(A)) {
                    var c = B.ptType ? B.ptType : null,
                      u = !!B.autoH;
                    if (
                      ((A = A.add(P)),
                      m && (A = A.subtract(m)),
                      (f = Math.abs(S.getX() - A.getX())),
                      (null === y && f <= n) ||
                        (null !== y && f < Math.abs(S.getX() - y)))
                    ) {
                      ((y = A.getX()), (C = f));
                      var d = c ? R(A, c, u) : A;
                      v =
                      ...
                      null !== y &&
                        ...
                        (C = f),
                        S.getY() <= A.getY()
                          ? (v[1].pt ? v[1].pt.getY() : v[1].getY()) <
                              A.getY() && (v[1] = c ? R(A, c, u) : A)
                          : (v[0].pt ? v[0].pt.getY() : v[0].getY()) >
                              A.getY() && (v[0] = c ? R(A, c, u) : A));
                  }
                }
              var E = new p(
                s.getX() - n,
              ...
              if (
                (x && (E = E.intersected(x)),
                r && (i = a ? a.findPivots(E) : e.findPivots(!1, E)),
                i)
              )
                for (t = 0; t < i.length; ++t) {
                  var B;
                  ...
                  if (r || E.containsPoint(A)) {
                    ...
                    if (
                      ((A = A.add(P)),
                      m && (A = A.subtract(m)),
                      (f = Math.abs(S.getY() - A.getY())),
                      (null === _ && f <= n) ||
                        (null !== _ && f < Math.abs(S.getY() - _)))
                    ) {
                      ((_ = A.getY()), (w = f));
                      d = c ? R(A, c, u) : A;
                      b =
                      ...
                      null !== _ &&
                        ...
                        (w = f),
                        S.getX() <= A.getX()
                          ? (b[1].pt ? b[1].pt.getX() : b[1].getX()) <
                              A.getX() && (b[1] = c ? R(A, c, u) : A)
                          : (b[0].pt ? b[0].pt.getX() : b[0].getX()) >
                              A.getX() && (b[0] = c ? R(A, c, u) : A));
                  }
                }
            }.bind(this),
            k = function (e, t) {
              var i = [];
              if (!e.hasMixin(h.Container)) return [];
              if (e.hasMixin(s.Accelerated))
                i =
                  e.retrieveChildrenInGeometryBBox(
                    t,
                    d.RETRIEVE_MODE_INTERSECT,
                  ) || [];
              else
          ...
          if (
            ...
            for (var N = 0; N < M.length; N++) {
              ...
              if (U instanceof a && !U.hasFlag(s.Flag.Hidden)) {
                var V = null,
                  O = this._getEffectiveSnapElems();
                if (O)
                  for (var L = 0; L < O.length && !V; ++L) U == O[L] && (V = U);
                else V = U;
                V && D(V);
              }
            }
