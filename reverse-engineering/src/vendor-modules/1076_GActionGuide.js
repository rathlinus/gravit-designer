        ...
        (c.prototype._gatherActions = function (e, t) {
          ...
          if (p) {
            ...
            var d = [],
              ...
              f = function (e) {
                if (
                  (e.hasMixin(h) && (d.push(e), g.push(u)),
                  e instanceof a && e.hasMixin(o.Container))
                )
                  if (e.hasMixin(a.Accelerated)) {
                    var t = e.retrieveChildrenInPaintBBox(
                      c,
                      A.RETRIEVE_MODE_INTERSECT,
                    );
                    t && t.forEach(f);
                  } else
                    for (
                      var i = e.getLastChild();
                      null != i;
                      i = i.getPrevious()
                    )
                      f(i);
            ...
            for (var m = [], y = 0; y < i.length; ++y)
              for (var _ = i[y], v = d.length - 1; v >= 0; v--) {
                ...
                if (w) {
                  ...
                  if (
                    ((w = w.translated(B.getX(), B.getY())),
                    _.containsRect(w) || w.intersectsRect(_))
                  ) {
