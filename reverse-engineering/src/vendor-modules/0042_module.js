...
function (e, t, i) {
      ...
      function c(e, t, i) {
        ((this.vertexAttribute = null),
          ...
          (t = t || g),
          (i = "precision highp float;" + (i = i || f)),
          (this.program = c.getLinkedProgram(t, i)));
      }
      ((c.shaderCache = {}),
        (c.linkedShaderCache = {}),
      ...
      ((u = !0),
        ...
        (c.TOLOCAL =
          "    vec2 toLocal(vec2 coord) {        return ((coord * texSize) - offset)/tileSize;    }"),
        (c.sobelVertexShader =
          "     attribute vec2 vertex;     attribute vec4 _texCoord;     uniform highp float texelWidth;     uniform highp float texelHeight;     varying highp vec2 texCoord;     varying vec2 leftTextureCoordinate;     varying vec2 rightTextureCoordinate;     varying vec2 topTextureCoordinate;     varying vec2 topLeftTextureCoordinate;     varying vec2 topRightTextureCoordinate;     varying vec2 bottomTextureCoordinate;     varying vec2 bottomLeftTextureCoordinate;     varying vec2 bottomRightTextureCoordinate;     void main()     {         gl_Position = vec4(vertex * 2.0 - 1.0, 0.0, 1.0);         vec2 widthStep = vec2(texelWidth, 0.0);         vec2 heightStep = vec2(0.0, texelHeight);         vec2 widthHeightStep = vec2(texelWidth, texelHeight);         vec2 widthNegativeHeightStep = vec2(texelWidth, -texelHeight);         texCoord = _texCoord.xy;         leftTextureCoordinate = _texCoord.xy - widthStep;         rightTextureCoordinate = _texCoord.xy + widthStep;         topTextureCoordinate = _texCoord.xy - heightStep;         topLeftTextureCoordinate = _texCoord.xy - widthHeightStep;         topRightTextureCoordinate = _texCoord.xy + widthNegativeHeightStep;         bottomTextureCoordinate = _texCoord.xy + heightStep;         bottomLeftTextureCoordinate = _texCoord.xy - widthNegativeHeightStep;         bottomRightTextureCoordinate = _texCoord.xy + widthHeightStep;     }"),
        (c.tilingAwareVertexSource =
          "    attribute vec2 vertex;    attribute vec2 _texCoord;    varying highp vec2 texCoord;    varying vec2 localTexCoord;    uniform vec2 tileSize;    uniform vec2 offset;    uniform vec2 texSize;    void main() {        localTexCoord = _texCoord;        texCoord = (localTexCoord * tileSize + offset)/texSize;        gl_Position = vec4(vertex * 2.0 - 1.0, 0.0, 1.0);    }"),
        (c.prototype._fxaaPremultiplied = null),
        (c.prototype._fxaaUnpremultiplied = null),
        (c.prototype.fxaa = function (e, t, i, n, r) {
          var o;
          if (!(o = r ? this._fxaaUnpremultiplied : this._fxaaPremultiplied))
            try {
              o = new c(
                ...
                c._fxaaVertexSource,
                r
                  ? c._fxaaFragmentSourceUnpremultiplied
                  : c._fxaaFragmentSourcePremultiplied,
              );
            } catch (e) {
              r
                ? (this._fxaaUnpremultiplied = {})
                : (this._fxaaPremultiplied = {});
            }
          o instanceof c &&
            (r
              ? (this._fxaaUnpremultiplied = o)
              : (this._fxaaPremultiplied = o),
            o.simpleShader.call(
              o,
              {
                texSize: [i || this.glEffect.width, n || this.glEffect.height],
        ...
        (c.screenSpaceDither =
          " vec3 screenSpaceDither (vec2 vScreenPos) {    highp vec3 vDither = vec3( dot( vec2( 171.0, 231.0 ), vScreenPos.xy ) );    vDither.rgb = fract( vDither.rgb / vec3( 103.0, 71.0, 97.0 ) );    return vDither.rgb / 255.0;}"),
        (c.dithering =
      ...
      var g =
          "    attribute vec2 vertex;    attribute vec2 _texCoord;    varying highp vec2 texCoord;    void main() {        texCoord = _texCoord;        gl_Position = vec4(vertex * 2.0 - 1.0, 0.0, 1.0);    }",
        f =
          "    uniform highp sampler2D texture;    varying highp vec2 texCoord;    void main() {        gl_FragColor = texture2D(texture, texCoord);    }";
      ...
      ((c.warpShader = function (e, t) {
        return (
          e +
          "    uniform highp sampler2D texture;    uniform vec2 texSize;    uniform vec2 tileSize;    uniform vec2 offset;    varying highp vec2 texCoord;    void main() {        highp vec2 coord = texCoord * tileSize + offset;        " +
          t +
          "        gl_FragColor = texture2D(texture, (coord - offset) / tileSize);        highp vec2 clampedCoord = clamp(coord, offset, tileSize+offset);        if (coord != clampedCoord) {            /* fade to transparent if we are outside the image */            gl_FragColor.a *= max(0.0, 1.0 - length(coord - clampedCoord));        }    }"
        );
      }),
        ...
        (c.prototype.simpleShader = function (e, t, i, n) {
          ((t || this.glEffect.texture).use(),
            this.glEffect.spareTexture.drawTo(
              function () {
        ...
        (c._getShaderCache = function () {
          return l.isRenderPhase()
            ...
            : (c.shaderCacheNoRender || (c.shaderCacheNoRender = []),
              c.shaderCacheNoRender);
        }),
        (c._getLinkedShaderCache = function () {
          return l.isRenderPhase()
            ? (c.linkedShaderCache || (c.linkedShaderCache = []),
              c.linkedShaderCache)
            : (c.linkedShaderCacheNoRender ||
                (c.linkedShaderCacheNoRender = []),
              c.linkedShaderCacheNoRender);
        ...
        (c.compileSource = function (e, t, i) {
          ...
          if (h)
            for (var A = 0; A < h.length; A++)
              if ((a = h[A]).src == t) return a.program;
          var p = r.createShader(e);
          if (
            (r.shaderSource(p, t),
            r.compileShader(p),
            !r.getShaderParameter(p, r.COMPILE_STATUS))
          )
        ...
        (c.getLinkedProgram = function (e, t) {
          ...
          if (!s) {
            ...
            return (
              l.attachShader(h, c.compileSource(l.VERTEX_SHADER, e, r)),
              l.attachShader(h, c.compileSource(l.FRAGMENT_SHADER, t, a)),
              c.linkProgram(h, r, a),
              h
            );
          }
        ...
        (c.prototype.destroy = function () {
          (this._fxaaUnpremultiplied &&
            this._fxaaUnpremultiplied instanceof c &&
            this._fxaaUnpremultiplied.destroy(),
            this._fxaaPremultiplied &&
              this._fxaaPremultiplied instanceof c &&
              this._fxaaPremultiplied.destroy());
        }),
              ...
              if (null !== r) {
                ...
                if (
                  ...
                  switch (a.length) {
                    case 1:
                      s
                        ? i.uniform1fv(r, new Float32Array(p(a)))
                        : i.uniform1iv(r, new Int32Array(p(a)));
                      break;
                    case 2:
                      s
                        ? i.uniform2fv(r, new Float32Array(p(a)))
                        : i.uniform2iv(r, new Int32Array(p(a)));
                      break;
                    case 3:
                      s
                        ? i.uniform3fv(r, new Float32Array(p(a)))
                        : i.uniform3iv(r, new Int32Array(p(a)));
                      break;
                    case 4:
                      s
                        ? i.uniform4fv(r, new Float32Array(p(a)))
                        : i.uniform4iv(r, new Int32Array(p(a)));
                      break;
                    case 9:
                ...
                else if (m(a)) s ? i.uniform1f(r, a) : i.uniform1i(r, a);
                else if (
                  !(
                    r instanceof h.UniformLocation ||
                    r instanceof A.UniformLocation
                  )
                )
                  throw (
        ...
        (c.prototype.textures = function (e) {
          ...
          for (var i in (t.useProgram(this.program), e))
            e.hasOwnProperty(i) &&
              t.uniform1i(t.getUniformLocation(this.program, i), e[i]);
          return this;
        }),
        (c.prototype.drawRect = function (e, t, i, n, r) {
          var a = o.getGLContext();
          a &&
            ((t = void 0 !== t ? (t - r[1]) / r[3] : 0),
            (e = void 0 !== e ? (e - r[0]) / r[2] : 0),
            (i = void 0 !== i ? (i - r[0]) / r[2] : 1),
            (n = void 0 !== n ? (n - r[1]) / r[3] : 1),
            null == a.vertexBuffer && (a.vertexBuffer = a.createBuffer()),
            a.bindBuffer(a.ARRAY_BUFFER, a.vertexBuffer),
            a.bufferData(
              a.ARRAY_BUFFER,
              new Float32Array([e, t, e, n, i, t, i, n]),
              a.STATIC_DRAW,
            ),
        ...
        (c.getDefaultShader = function () {
          ...
          return e
            ? ((e.defaultShader = e.defaultShader || new c()), e.defaultShader)
            : {
                drawRect: function () {},
              };
        }),
        (c._getFinalArea = function (e, t) {
          ...
          if (i) {
            ...
            if (!n || n.getWidth() < 1 || n.getHeight() < 1) return null;
          } else n = t;
          return n;
        }),
        (c._renderTile = function (e, t, i, n, r, a, l, h) {
          var A = o.getGLContext(),
            c = this._getFinalArea(
              e,
              n.translated(-a.getX(), -a.getY()).toAlignedRect(),
            );
          if (!c) return !1;
          var p,
            g,
          ...
          if (
            (t &&
              ...
              (f = i.getWidth()),
              (m = i.getHeight())),
            c.getX() >= a.getWidth() + a.getX() ||
              c.getY() >= a.getHeight() + a.getY())
          )
            return !1;
          if (
            a.getX() > c.getX() + c.getWidth() ||
            a.getY() > c.getY() + c.getHeight()
          )
            return !1;
          ...
          (A.clearColor(0, 0, 0, 0),
            ...
            d
              ? ((A.canvas.width = s.nextPowerOf2(y)),
                (A.canvas.height = s.nextPowerOf2(_)))
              : u
                ? (A.canvas.width !== Math.ceil(y) &&
                    (A.canvas.width = Math.ceil(y)),
                  A.canvas.height !== Math.ceil(_) &&
                    (A.canvas.height = Math.ceil(_)))
                : (A.canvas.width < s.nextPowerOf2(y) &&
                    (A.canvas.width = s.nextPowerOf2(y)),
                  A.canvas.height < s.nextPowerOf2(_) &&
          ...
          if (
            (P.draw(S), E.render(B, x, v, b, C, w), P.update(), S.destroy(), t)
          ) {
      ...
      ((c.shouldEnableTiles = function (e, t) {
        ...
        (c._getMaxSize = function (e, t) {
          ...
          if (e.getAreas()) {
            for (var r = 0, o = e.getAreas(), a = 0; a < o.length; a++)
              r = Math.max(
                r,
                o[a].getWidth() + n[0] + n[2],
                o[a].getHeight() + n[1] + n[3],
              );
            i = Math.max(256, Math.min(r, 1024));
          } else i = 1024;
          return (
            i < 2048 &&
              (n[0] + n[2] > 0.33 * i || n[1] + n[3] > 0.33 * i) &&
              (i *= 2),
            i
          );
        }),
        (c.computeDownScale = function (e, t, i) {
          ...
          var r = t.slice(),
            o = this._getMaxSize(e, t);
          (r[0] + r[2] > 0.33 * o || r[1] + r[3] > 0.33 * o) &&
            (n *= (0.33 * o) / Math.min(r[0] + r[2], r[1] + r[3]));
          return n;
        }),
        (c.apply = function (e, t, i, n, s, l, A) {
          ...
          if (e instanceof r) {
            ...
            var b = new a(
              e.getOffset().getX() - s.getX(),
              e.getOffset().getY() - s.getY(),
              e.getWidth(),
              e.getHeight(),
            );
            ((s = new a(
            ...
            0;
            var w =
              (n.shader.glEffect.getAbsoluteEffectPadding &&
                n.shader.glEffect.getAbsoluteEffectPadding()) ||
              0;
            w instanceof Array || (w = [w, w, w, w]);
            var E = this._getMaxSize(e, w),
              B = A || this.computeDownScale(e, w);
            ((w = w.map(function (e) {
            ...
            if (C) {
              ...
              for (var R = [], D = []; F < P; ) {
                for (
                  T = E - w[1] - w[3],
                    F + E >= P ? (T = P - F) : 0 === F && (T += w[1]),
                    T < 1 && (T = 1),
                    I = 0;
                  I < x;
                ) {
                  ((S = E - w[0] - w[2]),
                    I + E >= x ? (S = x - I) : 0 === I && (S += w[0]),
                    S < 1 && (S = 1));
                  var k = new a(I, F, E, E),
                    G = new a(
                      0 === I ? 0 : I + w[0],
            ...
            var j = s ? s.getWidth() : t,
              J = s ? s.getHeight() : i;
            if (
              (_.draw(g),
              f.render(m, y * (i / J), 0, 0, j, J),
              _.update(),
              g.destroy(),
              e instanceof h.ImageDataArray)
            )
        ...
        (c._genAreas = function (e, t, i, n) {
          ...
          return e.map(function (e) {
            return (
              (e = e.translated(r, o).scaled(rescale, rescale)),
              n || (e = e.translated(-r * rescale, -o * rescale)),
              e
            );
          });
        }),
        (c._updateAreas = function (e, t, i, r, o, s) {
          ...
          if (!n.equals(l, h)) {
            ...
            (e.resetClip(),
              ...
              e.setOrigin(A),
              e.setOffset(A),
              e.setScale(t.getScale() * o));
          }
        }),
        (c._copyCanvas = function (e, t, i, n, o, s) {
          var l = new r();
          s
            ? l.resize(n.getWidth(), n.getHeight())
            : l.resize(o.getWidth(), o.getHeight());
          var h = e.getAreas(),
            A = t.getSide(a.Side.TOP_LEFT),
            c = h
              ? h.map(function (e) {
                  var r;
                  return (
                    ((r = s
                      ? (r = e.translated(i.getX(), i.getY())).scaled(
                          n.getWidth() / t.getWidth(),
                          n.getHeight() / t.getHeight(),
                        )
                      : e.scaled(
                          o.getWidth() / i.getWidth(),
                          o.getHeight() / i.getHeight(),
                        )).getX() < 0 ||
                      r.getY() < 0) &&
                      (r = new a(
                        Math.max(0, r.getX()),
                        Math.max(0, r.getY()),
                        r.getWidth() + Math.min(0, r.getX()),
                        r.getHeight() + Math.min(0, r.getY()),
                      )),
                    r.toAlignedRect()
                  );
                })
          ...
          if ((l.prepare(c, !1), h))
            for (
              var p = new a(0, 0, e.getWidth(), e.getHeight()),
                u = new a(0, 0, l.getWidth(), l.getHeight()),
                d = 0;
              d < h.length;
              d++
            ) {
              var g = h[d].intersected(p),
                f = c[d].intersected(u);
              g.isEmpty() ||
                f.isEmpty() ||
                l.drawImageFragment(
