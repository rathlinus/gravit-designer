...
function (e, t, i) {
      ...
      (n.inheritAndMix(c, r, [r.Visual, r.Map, r.DetailMap]),
        ...
        (c.prototype._limitGridSize = function (e, t) {
          var i = e * t;
          return (
            Math.round(i) < c.MIN_CELL_SPACE &&
              (i *= 1 + Math.floor(c.MIN_CELL_SPACE / i)),
            i
          );
        }),
        (c.prototype.paint = function (e, t) {
          ...
          if (i && t.configuration.gridVisible) {
            var n = e.getScaleFactor(),
              r = this._scene.getProperty("gx"),
              A = this._scene.getProperty("gy"),
              c = this._scene.getProperty("gaw"),
              p = this._scene.getProperty("gah"),
              u = this._limitGridSize(r, n),
              d = this._limitGridSize(A, n),
              g = this._limitGridSize(c, n),
              f = this._limitGridSize(p, n);
            ((r = u / n), (A = d / n), (c = g / n), (p = f / n));
            var m = t.dirtyMatcher.getDirtyRectangles();
            if (i === o.GridMode.Axonometric && m.length > 1) {
              for (var y = m[0], _ = 1; _ < m.length; ++_) y = y.united(m[_]);
              m = [y];
            }
            var v,
              b,
            ...
            for (_ = S ? 0 : 1; _ < m.length; ++_) {
              ...
              var F = (x = e.mapRect(v));
              i === o.GridMode.Boxed
                ? ((b = new a(v.getX(), v.getY())),
                  (C = Math.round(b.getX() / r) * r),
                  (w = Math.round(b.getY() / A) * A),
                  (E = new a(C, w)),
                  (B = e.mapPoint(E)),
                  (F = new l(
                    Math.round(B.getX()),
                    Math.round(B.getY()),
                    x.getWidth() + x.getX() - Math.round(B.getX()),
                    x.getHeight() + x.getY() - Math.round(B.getY()),
                  )))
                : i === o.GridMode.Axonometric &&
                  (F = new l(
                    Math.round(x.getX()),
                    Math.round(x.getY()),
                    Math.round(x.getWidth() + x.getX() - Math.round(x.getX())),
                    Math.round(x.getHeight() + x.getY() - Math.round(x.getY())),
                  ));
            ...
            for (_ = 0; _ < m.length; ++_) {
              if (
                ...
                (x = e.mapRect(v)),
                (b = new a(v.getX(), v.getY())),
                (C = Math.round(b.getX() / T) * T),
                (w = Math.round(b.getY() / I) * I),
                (E = new a(C, w)),
                (B = e.mapPoint(E)),
                i === o.GridMode.Boxed && _ > 0
                  ? t.canvas.clearRect(
                      Math.round(B.getX()),
                      Math.round(B.getY()),
                      x.getWidth() + x.getX() - Math.round(B.getX()),
                  ...
                  : i === o.GridMode.Axonometric &&
                    t.canvas.clearRect(
                      ...
                      Math.round(x.getY()),
                      Math.round(
                        x.getWidth() + x.getX() - Math.round(x.getX()),
                      ),
                      Math.round(
                        x.getHeight() + x.getY() - Math.round(x.getY()),
                      ),
                    ),
              ...
              if (i === o.GridMode.Boxed) {
                ...
                var Q,
                  M = this._scene.getProperty("ga1"),
                  N = Math.tan(-M),
                  U = this._scene.getProperty("ga2"),
                  V = Math.tan(-U);
                ...
                if (((B = e.mapPoint(new a(0, 0))), !s.isEqualEps(N, 0)))
                  if (
                    ...
                    (G = Q.getY()) > x.getY() &&
                      (G -= f * Math.floor((G - x.getY()) / f));
                    var O = new a(Q.getX(), G);
                    if (
                      (X = s.getLinesIntersection(
                        N,
                        -1,
                        x.getY() +
                          x.getHeight() -
                      ...
                      for (G = O.getY(); G < X.getY(); G += f) {
                        ...
                        if (
                          ...
                          L.getY() > x.getY() + x.getHeight() &&
                            (L = s.getLinesIntersection(
                              N,
                              -1,
                              L.getY() - N * L.getX(),
                              0,
                ...
                if (!s.isEqualEps(V, 0))
                  if (
                    ...
                    (G = Q.getY()) < x.getY() + x.getHeight() &&
                      (G += f * Math.floor((x.getY() + x.getHeight() - G) / f));
                    var X;
                    O = new a(Q.getX(), G);
                    if (
                      (X = s.getLinesIntersection(
