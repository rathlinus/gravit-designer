...
function (e, t, i) {
      ...
      (g.inheritAndMix(x, a, [B]),
        ...
        (x._SmoothingManager.prototype._fitter = null),
        (x._SmoothingManager.prototype._firstTangent = null),
        (x._SmoothingManager.prototype._lastTangent = null),
        (x._SmoothingManager.prototype._cdist = 0),
        (x._SmoothingManager.prototype._path = null),
        (x._SmoothingManager.prototype._lastPos = null),
        (x._SmoothingManager.prototype._lastSmoothed = null),
        (x._SmoothingManager.prototype._startingPoint = null),
      ...
      ((x._SmoothingManager.prototype.start = function (e) {
        ...
        (x._SmoothingManager.prototype.move = function (e) {
          ...
          if (null == this._current || t >= S) {
            (this._currentPoints.length === P ||
              (this._currentPoints.length > 4 && this._cdist > T)) &&
              this._smoothCurrent(e);
            var n = new c.AnchorPoint();
            (n.setProperties(["x", "y"], [e.getX(), e.getY()]),
              this._path.getAnchorPoints().appendChild(n),
        ...
        (x._SmoothingManager.prototype._smoothCurrent = function (e) {
          ...
          if (this._currentPoints.length) {
            if (this._lastSmoothed) {
              ...
              ((this._firstTangent = this._currentPoints[0].subtract(n)),
                (this._lastTangent =
                  this._currentPoints[this._currentPoints.length - 1].subtract(
                    i,
                  )));
              var r = this._lastTangent.dot(this._lastTangent);
              0 !== r &&
                (this._lastTangent = this._lastTangent.scale(1 / Math.sqrt(r)));
            } else
              ((t = !0),
                this._currentPoints.length && this._startingPoint
                  ? (this._firstTangent = this._currentPoints[0].subtract(
                      this._startingPoint,
                    ))
                  : (this._firstTangent = new o(0, 0)));
            var a,
              s = this._firstTangent.dot(this._firstTangent);
            if (
              (0 !== s &&
                (this._firstTangent = this._firstTangent.scale(
                  1 / Math.sqrt(s),
                )),
              this._currentPoints.length > 2)
            ) {
              for (
                var l,
                  h,
                  A,
                  c = this._path.getAnchorPoints(),
                  p = this._lastSmoothed
                    ? this._lastSmoothed.getNext()
                    : c.getFirstChild();
                p;
              )
                ((l = p.getNext()), c.removeChild(p), (p = l));
              if (
                ((a = this._fitter.fitCurve(
                  this._currentPoints,
                  P,
                  this._firstTangent,
                  this._lastTangent,
                ...
                for (var u = 0; u < a.length; u++)
                  ((h = a[u]),
                    A.setProperties(["hrx", "hry"], [h[1].getX(), h[1].getY()]),
                    (A = new f.AnchorPoint()).setProperties(
                      ["x", "y", "hlx", "hly"],
                      [h[3].getX(), h[3].getY(), h[2].getX(), h[2].getY()],
                    ),
                    c.appendChild(A));
              } else if (a) {
                A = c.getLastChild();
        ...
        (x.prototype.activate = function (e, t) {
          (a.prototype.activate.call(this, e, t),
            t ||
              (e.addEventListener(r.Down, this._mouseDown, this),
              e.addEventListener(r.Release, this._mouseRelease, this),
              e.addEventListener(r.DragEnd, this._mouseDragEnd, this),
              e.addEventListener(s.Down, this._keyDown, this),
              e.addEventListener(r.DragStart, this._mouseDragStart, this),
              e.addEventListener(r.Drag, this._mouseDrag, this),
              e.addEventListener(r.Move, this._mouseMove, this),
        ...
        (x.prototype.deactivate = function (e, t) {
          (this._finishTransaction(),
            ...
            e.removeEventListener(r.Down, this._mouseDown),
            e.removeEventListener(r.Release, this._mouseRelease),
            e.removeEventListener(s.Down, this._keyDown),
            e.removeEventListener(r.Drag, this._mouseDrag),
            e.removeEventListener(r.DragStart, this._mouseDragStart, this),
            e.removeEventListener(r.DragEnd, this._mouseDragEnd, this),
            e.removeEventListener(r.Move, this._mouseMove),
            e.removeEventListener(r.DblClick, this._mouseDblClick),
            h.removeEventListener(A, this._modifiersChanged, this));
        }),
        ...
        (x.prototype._createAndAppendPath = function () {
          ...
          return (
            ...
            this._pathEditor.setFlag(l.Flag.Detail),
            e
          );
        }),
        (x.prototype._reset = function () {
          ((this._createdPath = this._newPath),
            (this._newPath = null),
        ...
        (x.prototype._keyDown = function (e) {
          if ((a.prototype._keyDown.call(this, e), e.key === d.Constant.ESC))
            ...
            ("string" == typeof e.key || e.key instanceof String) &&
            e.key.match(/^[0-9]$/)
          ) {
            var t = parseInt(e.key),
              i = this._newPath || this._createdPath;
            if (i) {
              var n = 0;
        ...
        (x.prototype._finishTransaction = function () {
          this._transactionStarted &&
            (this._editor.commitTransaction(
              w.get(new E("GFreehandTool", "action.create-freehand-path")),
            ),
          ...
          if (e.button === r.BUTTON_LEFT)
            if (h.modifiers.shiftKey) this._released = !1;
              ...
              (this._pathEditor && this._pathEditor.blockRemoval(),
                ...
                (this._startingSelection = this._editor.getSelection()),
                this._startingSelection &&
                  (this._startingSelection = this._startingSelection.slice()));
              var t = this._view
              ...
              if (this._lastReleasedPos) {
                for (
                  var n = Date.now(), o = this._lastReleasedPos.length - 2;
                  o >= 0;
                  o--
                ) {
                  var a = this._lastReleasedPos[o];
                  if (!(n - a.time < 500)) break;
              ...
              (this._mouseMove(e),
                ...
                (t = this._editor
                  .getGuides()
                  .mapPoint(t, m.DetailMap.Mode.DetailOnFilterOn)),
                ...
                (this._newPath = this._createAndAppendPath()),
                this._newPath &&
                  (this._smoothingManager.begin(this._newPath),
                  this._smoothingManager.start(i),
                  this._smoothingManager.move(t),
                  this._pathEditor.selectOnePoint(
                    this._newPath.getAnchorPoints().getFirstChild(),
                  ),
        ...
        (x.prototype._modifiersChanged = function (e) {
          e.changed.shiftKey &&
            ...
            (h.modifiers.shiftKey
              ...
              : this._released &&
                ((this._released = !1), this._mouseRelease(e)));
        }),
        (x.prototype._mouseMove = function (e) {
          ((this._lastMouseEvent = e),
            this._released
              ? (this._lastReleasedPos || (this._lastReleasedPos = []),
                this._lastReleasedPos.push({
                  pt: this._view
                    .getViewTransform(this._view.getScene().getActivePage())
                    .mapPoint(e.client),
                  time: Date.now(),
                }),
                this._lastReleasedPos.length > 10 &&
                  this._lastReleasedPos.shift(),
                h.modifiers.shiftKey && this._mouseDrag(e))
              : e.button == r.BUTTON_RIGHT && this._mouseDrag(e));
        }),
          ...
          if (this._newPath) {
            ...
            if (((this._lastMouseEvent = e), h.modifiers.shiftKey))
              this._newPath
                .getAnchorPoints()
                .getLastChild()
                .setProperties(["x", "y"], [t.getX(), t.getY()]);
            else this._smoothingManager.move(t);
            this._closeIfNeeded(1)
              ? this._newPath.hasFlag(n.Flag.Highlighted)
                ? this._pathEditor.requestInvalidation()
                : this._newPath.setFlag(n.Flag.Highlighted)
              : this._newPath &&
                this._newPath.hasFlag(n.Flag.Highlighted) &&
                this._newPath.removeFlag(n.Flag.Highlighted);
          }
        }),
            ...
            if (((this._lastMouseEvent = e), !this._released))
              try {
                ...
                if (i.getFirstChild() === i.getLastChild()) {
                  ...
                  (r.setProperties(
                    ["x", "y"],
                    [i.getFirstChild().$x + 1, i.getFirstChild().$y],
                  ),
                    i.appendChild(r));
                }
        ...
        (x.prototype._finalizeDrawing = function () {
          (this._closeIfNeeded() && this._correctFillVisibility(),
            this._newPath.setFlag(n.Flag.Selected));
        }),
