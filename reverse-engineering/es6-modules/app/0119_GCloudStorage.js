...
function (exports, module, require) {
  ...
  class G {
      ...
      return new Promise((l, GDocumentEvent) => {
        try {
          ...
          (0, GCloudImageSync.syncImagesToCloud)(
            ...
            GCore.GLocale.get(new GCore.GLocaleKey('GCommonNames', 'text.untitled-image')),
            GContainer,
            GLoginDialog,
            async (e, t) =>
    ...
    static resolveImage(e, t) {
      function require(e, t) {
        ...
        let CollaborationMergeUtils =
          AppSettings &&
          AppSettings.getDictionary() &&
          AppSettings.getDictionary().getEntry(require);
        return (
          ...
          (CollaborationMergeUtils.value.length > GEditor.GPlatform.maxImgDataUrlLength &&
            new g(
              GCore.GLocale.get(new GCore.GLocaleKey('GDocument', 'text.image-in-design-too-big'))
            ).open(),
      ...
      !(async function () {
        ...
        if (require(e))
          if (
            ...
            CollaborationMergeUtils.startsWith('123rf://')
          ) {
                    ...
                    (AppSettings.onload = function () {
                      if (this.status < 200 || this.status >= 400)
                        ...
                        if (n.size > GEditor.GPlatform.maxPngDataSize)
                          ...
                          ((AppSettings.onload = function () {
                            if (this.result.length > GEditor.GPlatform.maxImgDataUrlLength)
                              return void new g(
                                GCore.GLocale.get(
                                  new GCore.GLocaleKey('GDocument', 'text.image-in-design-too-big')
                                )
            ...
            (CollaborationMergeUtils.startsWith(GCore.GDictionary.PROTOCOL) &&
              ...
              'string' == typeof CollaborationMergeUtils &&
                ...
                (GCloudImageSync =
                  e.scene && e.scene.getDictionary() && e.scene.getDictionary().getEntry(e.url))),
            'string' == typeof CollaborationMergeUtils)
          ) {
            ...
            if (GDocumentEvent.has('id'))
              ((GAnnotationsUtils = GDocumentEvent.get('id')),
                (GContainer = GDocumentEvent.get('width')),
                (g = GDocumentEvent.get('height')));
              ...
              GAnnotationsUtils = CollaborationMergeUtils.slice(
                CollaborationMergeUtils.indexOf('://') + 3
              );
              ...
              ? AppSettings.gApi.getFile(GAnnotationsUtils).then((n) => {
                  GLoginDialog(
                    ...
                    (n) => {
                      (GCloudImageSync &&
                        t &&
                        ((GCloudImageSync.cloud = GCloudImageSync.value),
                        (GCloudImageSync.value = n)),
                        e.resolved(n));
                    },
                    true
                  );
          ...
          (AppSettings && (GDocumentEvent.width = AppSettings),
            ...
            (GDocumentEvent.onload = function () {
              ...
              ((GDocumentEvent.width > GEditor.GPlatform.maxImgLinearDimension ||
                ...
                GSystemDialog.drawImage(
                  GDocumentEvent,
                  0,
                  0,
                  GDocumentEvent.width,
                  GDocumentEvent.height
                ));
    ...
    static async updateFileThumbnail(e, t, n, GCore) {
      ...
      for (var l in GCloudImageSync)
        CollaborationMergeUtils.setRequestHeader(l, GCloudImageSync[l]);
    ...
    static isOnline() {
      return 'undefined' != typeof window && undefined !== window.gApi && window.gApi.url;
    }
    ...
    static unzipData(e) {
      ...
      try {
        ...
        if ('undefined' == typeof TextDecoder && e.length > 1e7) {
          for (
            var require = [], AppSettings = e.length, GCore = 0;
            GCore < AppSettings;
            GCore += 32768
          )
    ...
    static resendEmailConfirmation(e) {
      ...
      return AppSettings.gApi
        ...
        .catch((e) =>
          GSystemDialog.custom({
            title: GCore.GLocale.get(new GCore.GLocaleKey('GCommonNames', 'text.something-wrong')),
          ...
          switch (e.code) {
            case AppSettings.gApi.ERROR_CODES.ERR_SUBSCRIPTION_COULD_NOT_BE_DEACTIVATED:
              ...
              return (
                e
                  ...
                  .on(
                    'click',
                    (e) => (e.preventDefault(), gDesigner.runDeepLink('purchases'), false)
                  ),
            ...
            case AppSettings.gApi.ERROR_CODES.ERR_SUBSCRIPTION_IS_ACTIVE:
              ...
              return GSystemDialog.alert(
                GCore.GLocale.get(
                  new GCore.GLocaleKey('GCloudUtil', 'text.err-subscription-is-active')
                ).replace('%date', AppSettings.DateAPI.format(module))
              );
            case AppSettings.gApi.ERROR_CODES.ERR_SUBSCRIPTION_IS_NOT_EXPIRED:
              return GSystemDialog.alert(
                GCore.GLocale.get(
                  new GCore.GLocaleKey('GCloudUtil', 'text.err-subscription-is-not-expired')
                ).replace(
                  '%date',
            ...
            case AppSettings.gApi.ERROR_CODES.ERR_SUBSCRIPTION_IS_LIFETIME:
              return GSystemDialog.alert(
                GCore.GLocale.get(
                  new GCore.GLocaleKey('GCloudUtil', 'text.err-subscription-is-lifetime')
                )
              );
          }
