...
function (exports, module, require) {
  ...
  class u {
    ...
    _systemColorInput = null;
    _colorPreview = null;
    _colorPreviewOld = null;
    _colorPreviewNew = null;
    _opacitySlider = null;
    _palettes = null;
    _activePalette = null;
    _swatchesScope = 'user';
    _colorMode = null;
    _oldColor = GCore.GRGBColor.BLACK;
    _color = GCore.GRGBColor.BLACK;
    _oldColorOpacity = 100;
    _colorOpacity = 100;
    _activeGradientStop = null;
    _activeGradient = null;
    _pattern = null;
    _patternUpdateBlocker = false;
    _texture = null;
    _opacity = null;
    _settings = null;
    _isVisible = false;
    _extendedGamutInitiated = null;
    _sliderColorThumb = null;
    _sliderOpacityThumb = null;
    _extValue = null;
    ...
    initLayout() {
        ...
        .on(
          ...
          function (e, t, n) {
            if (
              ...
              (GCore &&
                (GCore.getEditor().removeEventListener(
        ...
        $('<select></select>')
          ...
          .on('change', (e) => {
            var t = $(e.target).children('option:selected').data('type'),
              n = $('.colormode-selector').children('option:selected').data('colormode'),
          ...
          .on(
            ...
            CollaborationMergeUtils.watchDog.trap(
              (e) => {
                ...
                var n = gDesigner.getActiveDocument().getColorModeElms() || [],
                  GTools = gDesigner.getActiveDocument().getEditor().getIndividualSelection();
                (GTools && (n.push(GTools[0]), gDesigner.getActiveDocument().setColorModeElms(n)),
                  this.setColorMode(t),
                  this._updateActiveGradient());
              },
              (e) => {
                ...
                return !!gDesigner.isEnabledProFeatures(t.toLowerCase()) || !!u.ColorModelFree[t];
              },
              (e) => {
                var t = $(e.target).children('option:selected').data('colormode');
          ...
          .append(
            $('<div/>')
              ...
              .on('mousemove', function (e) {
                e.originalEvent.isTrusted && (e.preventDefault(), e.stopPropagation());
              })
          )
          .append($('<div/>').addClass('marker'))
          .appendTo(this._colorEditor)));
      var n = $('<div/>').addClass('color-details').appendTo(this._colorEditor);
      $('<div/>')
          ...
          .append(
            $('<button />')
              ...
              .attr(
                'data-title',
                GCore.GLocale.get(
                  new GCore.GLocaleKey('GPatternChooser', 'action.change-stops-order')
                )
              )
              .on(
                'click',
                function () {
                  ...
                  for (var e = this._activeGradient.getStops(), t = 0; t < e.length; ++t)
                    ((e[t].position = 1 - e[t].position), this._updateGradientStop(e[t]));
                  this._updatePatternFromActiveGradient();
                }.bind(this)
              )
              ...
              .append(
                $('<button />')
                  ...
                  .attr(
                    'data-title',
                    GCore.GLocale.get(
                      new GCore.GLocaleKey('GPatternChooser', 'action.rotate-gradient-left')
                    )
                  )
                  .on(
                    'click',
              ...
              .append(
                $('<button />')
                  ...
                  .attr(
                    'data-title',
                    GCore.GLocale.get(
                      new GCore.GLocaleKey('GPatternChooser', 'action.rotate-gradient-right')
                    )
                  )
                  .on(
                    'click',
      ...
      var GSystemDialog = function (e) {
        ...
        (gDesigner.stats('patternchooser_activate_palette', t), this._activatePalette(t));
      }.bind(this);
    ...
    _createChoosers(e) {
      return $('<div/>')
        ...
        .append(
          $('<button />')
            .attr('data-palette', 'colors')
            .text(GCore.GLocale.get(new GCore.GLocaleKey('GCommonNames', 'text.colors')))
            .on('click', e)
        )
        .append(
          $('<button />')
            ...
            .attr('data-palette', 'swatches')
            .text(GCore.GLocale.get(new GCore.GLocaleKey('GPatternChooser', 'text.swatches')))
            .on(
              'click',
              CollaborationMergeUtils.watchDog.trap(
                e,
        ...
        .append(
          $('<button />')
            .attr('data-palette', 'used')
            .text(GCore.GLocale.get(new GCore.GLocaleKey('GPatternChooser', 'text.in-use')))
            .on('click', e)
        )
        .append(
          $('<button />')
            .attr('data-palette', 'mixer')
            .text(GCore.GLocale.get(new GCore.GLocaleKey('GPatternChooser', 'text.mixer')))
            .on('click', e)
        );
    }
    ...
    _updateActiveGradient() {
      if (this._activeGradient) {
        const e = this._activeGradient.getStops();
        if (e && e.length) {
          const t =
            u._ColorModeToFrameworkColorMode[this._colorMode] || GCore.GColor.ColorModes.RGB;
          (e.forEach((e) => {
    ...
    setColorMode(e, t) {
      ...
      if (e !== this._colorMode || t) {
        ...
        if (
          ...
          this._colorMode === u.ColorMode.RGB || this._colorMode === u.ColorMode.HSV)
        ) {
            ...
            $('<label />')
              ...
              .append(
                $('<input>')
                  ...
                  .on('long-press', (e) => {
                    n.isTouchEnabled &&
                      ((this._currentLongPressTarget = e.target), this._contextMenu.open(e.target));
                  })
                  .on(
                    'change',
                    function (t) {
                      ...
                      (n || (n = this._color),
                        e === u.ColorMode.HSV &&
                          (n = new GCore.GHSVColor(GCore.GColor.rgbToHSV(n.getValue()))),
                  ...
                  .on('paste keydown keyup', (e) => {
                    ...
                    if ('keydown' === e.type) {
                      ...
                      if (GTools.length >= 7 && GCore && n.selectionStart === n.selectionEnd)
                        return false;
                    }
              ...
              .append(
                $('<span />').text(
                  GCore.GLocale.get(new GCore.GLocaleKey('GPatternChooser', 'text.hex'))
                )
              )
          ...
          Array.prototype.forEach.call(
            ...
            function (e, t) {
              ...
              $('<label />')
                ...
                .append(
                  $('<input>').attr('type', 'text').gInputBox({ minValue: 0, maxValue: GTools })
                )
                .append($('<span />').text(e))
                .appendTo(this._colorComponents);
            }.bind(this)
          ...
          this._colorMode === u.ColorMode.CMYK &&
            ...
            this._color instanceof GCore.GCMYKColor ||
              (GTools = new GCore.GCMYKColor(GCore.GColor.rgbToCMYK(this._color.toScreen()))));
        ...
        (this._colorComponents.find('[data-component-index] input').each(function (e, t) {
          ...
          $('<label />')
            ...
            .append(
              $('<input>')
                .attr('type', 'text')
                .gInputBox({ minValue: 0, maxValue: 100, postfix: '%' })
                .gInputBox('value', GCore.GUtil.formatOpacity(100 * this._colorOpacity))
                .on(
                  'change',
                  function (e) {
                    (gDesigner.stats('patternchooser_change_opacity'),
                      this._updateOpacity(
                        GCore.GLength.parseEquationValue($(e.target).gInputBox('value')) / 100,
                        'input'
                      ));
                  }.bind(this)
                )
          ...
          GTools ? this._updateColor(GTools, 'mode') : this._updateColorComponentsFromColor(),
          this._relayout());
      }
    }
      ...
      (this._updateSettings(t, true),
        ...
        gDesigner.getWorkspace().getStyleEdManager() &&
          ...
          gDesigner
            .getWorkspace()
            .getStyleEdManager()
            .addEventListener(GTools.GStyleEdManager.EditorEvent, this._styleEditorHandler, this));
    ...
    _updateSettings(e, t) {
      ...
      (this._opacitySlider.gInputSlider(
        'disabled',
        this._settings.hasOwnProperty('hasOpacity') && !this._settings.hasOpacity
      ),
        this._patternEditor
    ...
    _createNoiseEditor() {
      ...
      $('<div/>')
        .gPropertyRow({
          label: GCore.GLocale.get(new GCore.GLocaleKey('GPatternChooser', 'text.intensity')),
          columns: [
            {
              width: 'auto',
              content: $('<div/>')
                ...
                .on(
                  ...
                  function (e) {
                    if ((gDesigner.stats('patternchooser_change_noise-amount'), this._pattern)) {
                      ...
                      (t.setAmount(parseFloat($(e.target).gInputSlider('value')) / 100),
      ...
      return (
        $('<div/>')
          .gPropertyRow({
            label: GCore.GLocale.get(new GCore.GLocaleKey('GPatternChooser', 'text.type')),
            columns: [
              {
                width: 'auto',
                ...
                Array.prototype.forEach.call(Object.keys(t), function (e) {
                  $('<option></option>')
                    ...
                    .text(
                      GCore.GLocale.get(
                        new GCore.GLocaleKey('GCommonNames', 'noise.' + n + '.' + t[e])
                      )
                    )
                    .appendTo(GTools);
                }),
                  ...
                  .on(
                    ...
                    function (e) {
                      if ((gDesigner.stats('patternchooser_change_noise', 'type'), this._pattern)) {
                        var t = this._pattern.clone();
                        (t.setType($(e.target).val()), this._updatePattern(t, 'noise_type'));
                      }
                    }.bind(this)
                  ),
              },
      ...
      return [
        {
          ...
          content: $(
            '<button>' +
              GCore.GLocale.get(new GCore.GLocaleKey('GPatternChooser', 'action.choose-image')) +
              '...</button>'
          )
        ...
        },
        {
          ...
          content: $('<button />')
            ...
            .append(
              $('<span />')
                .addClass('title')
                .html(GCore.GLocale.get(new GCore.GLocaleKey('GPasteAction', 'title')))
            )
            .on('click', t),
        },
      ];
      ...
      return [
        {
          ...
          content: $(
            '<button>' +
              GCore.GLocale.get(new GCore.GLocaleKey('GCommonNames', 'text.advanced')) +
              '</button>'
          ).on(
            'click',
            function () {
      ...
      var e = this.__getCreatePatternEditorParams(),
          ...
          return (
            ...
            Array.prototype.forEach.call(Object.keys(e), function (GTools) {
              $('<option></option>')
                ...
                .text(
                  GCore.GLocale.get(
                    new GCore.GLocaleKey('GCommonNames', 'texture.' + t + '.' + e[GTools])
                  )
                )
                .appendTo(n);
            }),
        ...
        CollaborationMergeUtils = (e) => {
          if ((gDesigner.stats('patternchooser_change_texture', 'tile'), this._pattern)) {
            var t = this._pattern.clone();
            (t.setTileSize(parseFloat($(e.target).gInputSlider('value')) / 100),
                ...
                columns: this._createPatternEditorFirstRow(
                  () => {
                    ...
                    var e = gDesigner.getActiveDocument(),
                      t = t || (e ? e.getStorage() : null) || gDesigner.getDefaultStorage();
                    t &&
                      t.canPromptOpen() &&
                      t.openPrompt(
                        u.enableFileTypes,
                        ...
                        function (e) {
                          if (this._pattern) {
                            ...
                            (t.setScaleMode($(e.target).val()),
                              (0, GCore.GTexturePattern.ScaleSettings[$(e.target).val()])(t),
      ...
      return (
        ...
        $('<div/>')
          .gPropertyRow({
            columns: [
              {
                label: GCore.GLocale.get(new GCore.GLocaleKey('GPatternChooser', 'text.repeat')),
                width: e.repeatWidth,
                content: n(GCore.GTexturePattern.RepeatMode, 'repeat')
                  ...
                  .on(
                    ...
                    function (e) {
                      if (
                        (gDesigner.stats('patternchooser_change_texture', 'repeat'), this._pattern)
                      ) {
                        var t = this._pattern.clone();
              ...
              { width: e.ghostWidth },
              {
                label: GCore.GLocale.get(new GCore.GLocaleKey('GCommonNames', 'text.position')),
                width: e.postionWith,
                content: n(GCore.GTexturePattern.PositionMode, 'position')
                  ...
                  .on(
                    ...
                    function (e) {
                      if (
                        (gDesigner.stats('patternchooser_change_texture', 'position'),
                        this._pattern)
                      ) {
                        var t = this._pattern.clone();
          ...
          .gPropertyRow({
            columns: [
              {
                label: GCore.GLocale.get(new GCore.GLocaleKey('GCommonNames', 'text.size')),
                width: e.sizeWidth,
                content: n(GCore.GTexturePattern.SizeMode, 'size')
                  ...
                  .on(
                    ...
                    function (e) {
                      if (
                        (gDesigner.stats('patternchooser_change_texture', 'size'), this._pattern)
                      ) {
                        var t = this._pattern.clone();
                        (t.setSizeMode($(e.target).val()), this._updatePattern(t, 'texture_size'));
                      }
                    }.bind(this)
                  ),
              },
                  ...
                  .on(
                    ...
                    function (e) {
                      if (
                        ...
                        var t = GCore.GLength.parseEquationValue($(e.target).val());
                        if (null !== t && t >= 0) {
              ...
              },
              {
                ...
                content: $('<input>')
                  ...
                  .on(
                    ...
                    function (e) {
                      if (
                        (gDesigner.stats('patternchooser_change_texture', 'size-h'), this._pattern)
                      ) {
                        var t = GCore.GLength.parseEquationValue($(e.target).val());
                        if (null !== t && t >= 0) {
                          var n = this._pattern.clone();
    ...
    _findOrCreateLastSwitchContiner(e) {
      var t = e.find('.swatches:last');
      t.length > 0
        ? t.find('.swatch').length >= 8 && (t = $('<div/>').addClass('swatches').appendTo(e))
        : (t = $('<div/>').addClass('swatches').appendTo(e));
      return t;
    }

    _createPaletteSwatch(e, t, n, GTools) {
      ...
      var GInputSliderWidget = function (e) {
          if (!u.canDragSwatch) return (e.preventDefault(), void e.stopPropagation());
        ...
        GSystemDialog = function (e) {
          ...
          if (
            ...
            try {
              ...
              g.commitTransaction(
                GCore.GLocale.get(new GCore.GLocaleKey('GPatternChooser', 'action.remove-swatch')),
                { chooserOn: true }
              );
            }
          }
        ...
        f = $('<div/>')
          ...
          .on(
            ...
            function (e) {
              ...
              } else (this._updateOpacity(GTools), this._updatePattern(n, 'set_type', true));
        ...
        n &&
          f
            ...
            .on('dragenter', function () {
              (function (e) {
                let t = u.dragSwatch.closest('.swatches-wrapper').hasClass('global'),
                  n = e.closest('.swatches-wrapper').hasClass('global'),
                  GTools = u.dragSwatch.data('swatch').isCMYK() === e.data('swatch').isCMYK();
            ...
            .on(
              ...
              function () {
                ...
                var e = u.dragSwatch.closest('.swatches-wrapper').find('.g-drop');
    ...
    __getCreateColorsPaletteParams() {
      return { isTouchEnabled: false };
    }

    _createColorsPalette(e) {
      ...
      for (let n = -1; n < GSwatchesChangedEvent.length; ++n) {
        ...
        t.isTouchEnabled || (GInputSliderWidget = $('<div/>').addClass('swatches').appendTo(e));
    ...
    _createMixerPalette(e) {
      for (
        var module = $('<div/>')
            ...
            .append(
              $('<label />').text(
                GCore.GLocale.get(new GCore.GLocaleKey('GPatternChooser', 'text.tints'))
              )
            )
            .appendTo(e),
          require = $('<div/>')
            ...
            .append(
              $('<label />').text(
                GCore.GLocale.get(new GCore.GLocaleKey('GPatternChooser', 'text.shades'))
              )
            )
            .addClass('swatches')
            .appendTo(e),
          GTools = $('<div/>')
            ...
            .append(
              $('<label />').text(
                GCore.GLocale.get(new GCore.GLocaleKey('GPatternChooser', 'text.tones'))
              )
            )
            .addClass('swatches')
            .appendTo(e),
          a = $('<div/>')
            ...
            .append(
              $('<label />').text(
                GCore.GLocale.get(new GCore.GLocaleKey('GPatternChooser', 'text.mixes'))
              )
            )
            .addClass('swatches')
            .appendTo(e),
    ...
    _updateMixerPalette() {
      ...
      for (
        var require = [255, 255, 255],
          ...
          GSystemDialog = this._palettes.find('.mixer-palette [data-container="mixes"] .swatch'),
          u = 0;
        u < e.maxCount;
        u += 1
      ) {
        ...
        (module(
          ...
          module(
            new GCore.GRGBColor(GCore.GRGBColor.blend(CollaborationMergeUtils, GTools, p)),
            GSwatchesChangedEvent,
            u
          ),
          module(
        ...
        a = $('<div/>')
          ...
          .append(
            $('<button/>')
              ...
              .attr(
                'data-title',
                GCore.GLocale.get(new GCore.GLocaleKey('GCommonNames', 'text.import-swatches'))
              )
              .addClass('swatch-icon')
              .addClass('gravit-icon-swatches-import')
              .on('click', () => {
                (gDesigner.stats('patternchooser_import_swatches'), gDesigner.importSwatches(t));
              })
          )
          .append(
            $('<button/>')
              ...
              .attr(
                'data-title',
                GCore.GLocale.get(new GCore.GLocaleKey('GCommonNames', 'text.export-swatches'))
              )
              .addClass('swatch-icon')
              .addClass('gravit-icon-swatches-export')
              .on('click', () => {
                (gDesigner.stats('patternchooser_export_swatches'), gDesigner.exportSwatches(t));
              })
          )
          .appendTo(e);
      ...
      return t && t instanceof GCore.GLinearGradient
        ...
        : t && t instanceof GCore.GRadialGradient
          ...
          : t && t instanceof GCore.GAngularGradient
            ? e + '-angular-gradient'
            : t && t instanceof GCore.GTexturePattern && !(t instanceof GCore.GNoisePattern)
              ? e + '-texture-pattern'
    ...
    _updateSwatchesPalette(e) {
      ...
      if (!t.isTouchEnabled && CollaborationMergeUtils.length)
        for (let e = 0; e < CollaborationMergeUtils.length; ++e)
          (GSwatchesChangedEvent ||
            (GSwatchesChangedEvent = $('<div/>').addClass('swatches').appendTo(GTools)),
            this._createPaletteSwatch(
              CollaborationMergeUtils[e],
              GSwatchesChangedEvent,
              true,
            ...
            14 == ++GInputSliderWidget &&
              ((GInputSliderWidget = 1), (GSwatchesChangedEvent = null)));
      if (
        (GSwatchesChangedEvent ||
          (GSwatchesChangedEvent = $('<div/>').addClass('swatches').appendTo(GTools)),
        $('<button />')
          ...
          .attr(
            'data-title',
            GCore.GLocale.get(new GCore.GLocaleKey('GPatternChooser', 'action.add-swatch'))
          )
    ...
    _activatePalette(e) {
      if (e !== this._activePalette) {
        ...
        switch (
          (this._palettes.find('.chooser [data-palette]').each(function (t, n) {
            var GTools = $(n);
    ...
    _setActiveGradientStop(e, t, n) {
      (e !== this._activeGradientStop || n) &&
        ((this._activeGradientStop = e),
        this._activeGradientStop &&
          (this._updateColor(t || this._activeGradientStop.color, n ? '' : 'gradient-stop'),
    ...
    _updateOnlineEditorStops() {
      if (
        ...
        var module = gDesigner.getWorkspace().getStyleEdManager().getActiveEditor(),
          require = null;
        (null !== exports &&
          module &&
            ...
            function (t) {
              ...
              n instanceof GCore.GCMYKColor
                ...
                : n instanceof GCore.GHSVColor
                  ...
                  : n instanceof GCore.GRGBColor && this.setColorMode(u.ColorMode.RGB);
            }.bind(this)
          )
          .appendTo(t)),
        this._updateGradientStop(e),
    ...
    _updatePatternFromActiveGradient(e) {
      if (this._activeGradient) {
        this._activeGradient.sortStops();
        var module = this._activeGradient.clone(),
          require = module.getStops();
        (Array.prototype.forEach.call(this._activeGradient.getStops(), function (e, t) {
        ...
        'mode' !== t &&
          (this._color instanceof GCore.GCMYKColor
            ...
            : this._color instanceof GCore.GHSVColor
              ? this.setColorMode(u.ColorMode.HSV)
              : this._color instanceof GCore.GRGBColor && this.setColorMode(u.ColorMode.RGB)),
    ...
    _updateOpacity(e, t, n, GTools) {
      ...
      if (e !== this._colorOpacity || 'set_opacity' === t || GTools) {
        ...
        (this._activeGradientStop ||
          ...
          this._colorPreviewNew.css('background', this._color.toScreenCSS(this._colorOpacity)),
          'slider' !== t &&
            this._opacitySlider.gInputSlider('value', Math.round(100 * this._colorOpacity)),
        ...
        if (a) {
          ...
          if (
            ...
            this._patternEditor
              ...
              .gInputSlider('disabled', e.getScaleMode() !== GCore.GTexturePattern.ScaleMode.Tile),
            this._patternEditor
              .find('[data-property="texture_size_u"]')
              .text(e.getSizeMode() === GCore.GTexturePattern.SizeMode.Length ? 'px' : '%'),
      ...
      if (this._colorMode === u.ColorMode.RGB || this._colorMode === u.ColorMode.HSV) {
        switch (this._colorMode) {
          ...
          case u.ColorMode.HSV:
            ((GTools = []),
              this._colorComponents.find('[data-component-index]').each(function (e, t) {
                var n = $(t),
                  GCore = parseInt(n.attr('data-component-index'));
                0 !== GCore && (GTools[GCore] = parseInt(n.find('input').gInputBox('value')) / 100);
              }));
        }
        GTools &&
    ...
    _updateColorFromColorComponents() {
      var e = [];
      this._colorComponents.find('[data-component-index]').each(function (t, n) {
        var GTools = $(n),
    ...
    _updateColorComponentsFromColor() {
      if (
        (this._colorComponents
          .find('[data-css]')
          .find('input')
    ...
    _updateColorMap() {
      ...
      this._colorMap.width() &&
        ...
        ((e.width = this._colorMap.width()), (e.height = this._colorMap.height()));
      var t = e.width,
        n = e.height,
        ...
        var module = function (e, t) {
            ...
            var a = this._colorMap.find('canvas')[0],
              ...
              GInputSliderWidget = Math.max(
                0,
                Math.min(CollaborationMergeUtils, Math.round(require - GSwatchesChangedEvent.left))
              ),
    ...
    static ColorMode = { RGB: 'rgb', HSV: 'hsv', CMYK: 'cmyk' };

    static _ColorModeToFrameworkColorMode = {
    ...
    static ColorModeLabel = { RGB: 'RGB', HSV: 'HSB', CMYK: 'CMYK' };

    static ColorModelFree = { RGB: true };

    static ExtendedGamut = { COMPONENTS: 'cp', COLOR_SLIDER: 'cs', MAP: 'map' };

    static canDragSwatch = false;

    static dragSwatch = null;

    static dragDeltaX = 0;

    static dragDeltaY = 0;

    static hasDropped = false;

    static EXTEND_DRAG_RANGE = 50;

    static enableFileTypes = [
    ...
    static PATTERN_TYPES = u.initPatternType();

    static dragSwatch = t;

    static hasDropped = false;

    static dragDeltaX = GTools.pageX - n.left;

    static dragDeltaY = GTools.pageY - n.top;

    static hasDropped = false;

    static canDragSwatch = $(e.target).hasClass('swatch');

    static _createPatternOption(e) {
    ...
    static getGradientStopsFromCurrentPattern(e) {
      var t;
      return (
    ...
    static PATTERN_TYPES = null;

    static initPatternType() {
      return [
        {
          name: GCore.GLocale.get(new GCore.GLocaleKey('GPatternChooser', 'pattern-type.color')),
        ...
        },
        {
          ...
          createDefault: function (e) {
            return new GCore.GLinearGradient(u.getGradientStopsFromCurrentPattern(e));
          },
        },
        {
          name: GCore.GLocale.get(
          ...
          createDefault: function (e) {
            return new GCore.GRadialGradient(u.getGradientStopsFromCurrentPattern(e));
          },
        },
        {
          name: GCore.GLocale.get(
            new GCore.GLocaleKey('GPatternChooser', 'pattern-type.angulargradient')
          ),
          ...
          createDefault: function (e) {
            return new GCore.GAngularGradient(u.getGradientStopsFromCurrentPattern(e));
          },
        },
        {
          name: GCore.GLocale.get(new GCore.GLocaleKey('GPatternChooser', 'pattern-type.texture')),
          cssBackgroundImage:
          ...
          isInstance: function (e) {
            return e && e instanceof GCore.GTexturePattern && !(e instanceof GCore.GNoisePattern);
          },
        ...
        {
          name: GCore.GLocale.get(new GCore.GLocaleKey('GPatternChooser', 'pattern-type.noise')),
          cssBackgroundImage:
            'url("data:image/svg+xml;base64,' +
            btoa(
    ...
    (function (e) {
      var t = {
        ...
        setPattern: function (t) {
          var n = e(this);
          return (gPatternChooser.isOpenned(n) && gPatternChooser.setPattern(t), this);
        },
      ...
      e.fn.gPatternChooser = function (n) {
        return t[n]
          ...
          : 'object' != typeof n && n
            ? void e.error('Method ' + n + ' does not exist on jQuery.gPatternChooser')
