...
function (exports, module, require) {
  ...
  const GSystemDialog = require(44) /* GSystemDialog */,
    ...
    y = ['text'];
  class v {
    static expanded = true;

    static expanded = true;

    static expanded = !v.expanded;
  ...
  function P() {
    return (
      ...
      (GSystemDialog.alert(
        GCore.GLocale.get(
          new GCore.GLocaleKey('GAnnotationPanel', 'text.document-approved-no-annotations-update')
        )
      ),
      false)
    );
  ...
  function L(e, t, n) {
    ...
    if (GInvalidationOptions) {
      ...
      if (
        ...
        !GCollaborationMentionsUtils.showResolved &&
          ((GInvalidationOptions.hasMixin(GCore.GAnnotation) &&
            ...
            (GInvalidationOptions instanceof GCore.GComment &&
              GInvalidationOptions.getParent().getProperty('rsv'))))
      )
      ...
      if (
        ...
        ).hasFlag(GCore.GNode.Flag.Selected) &&
          ((GInvalidationOptions.hasMixin(GCore.GAnnotation) && !B(GInvalidationOptions).length) ||
            U(GInvalidationOptions)) &&
          (p = true),
          ...
          P() &&
            (GInvalidationOptions.getProperty('text') === e
              ...
              : GTools.GEditor.tryRunTransaction(
                  ...
                  GCore.GLocale.get(new GCore.GLocaleKey('GAnnotationPanel', 'text.edit-comment'))
                ));
        },
        onToggleState: () => {
          P() &&
            (GTools.GEditor.tryRunTransaction(
              ...
              function () {
                (GCollaborationMentionsUtils.showResolved ||
                  ...
                  GInvalidationOptions.setProperty(
                    'rsv',
                    !GInvalidationOptions.getProperty('rsv')
                  ));
              },
              GInvalidationOptions.getProperty('rsv')
                ? GCore.GLocale.get(new GCore.GLocaleKey('GAnnotationPanel', 'text.reopen'))
                : GCore.GLocale.get(new GCore.GLocaleKey('GAnnotationPanel', 'text.resolve'))
            ),
            ne.call(this),
            te.call(this));
        },
      ...
      ((AppSettings.element = f),
        (AppSettings.component = w),
        GCollaborationMentionsUtils.vtree.addChild(w));
              ...
              try {
                if (t) {
                  ...
                  ((GCollaborationMentionsUtils.annotTreeNodeMap[n] = GInvalidationOptions),
                    GCollaborationMentionsUtils.replyNodes.set(
                  ...
                  GAnnotationReplyEditor =
                    GTools.GAnnotationEditor.createAddAnnotationTransactionData(
                      [t],
                      AppSettings.replyAnnot
                    );
                }
  ...
  function I(e, t) {
    ...
    let AppSettings = $('<div/>').addClass('already-assigned-user-row').appendTo($(this)),
    ...
    return (
      (0, GCollaborationMentionsUtils.getCollabInfo)(require[0]).then(async (n) => {
        ...
        $('<span/>')
          ...
          .html(
            GCore.GLocale.get(new GCore.GLocaleKey('GAnnotationPanel', 'text.assigned-to')) +
              (n && n.name
                ? GCollaborationMentionsUtils
                : GCore.GLocale.get(new GCore.GLocaleKey('GCommonNames', 'text.unknown-user')))
          )
          .appendTo(GInvalidationOptions);
        const AppSettings = !_interopRequireDefault;
        ((await AppSettings)
          ? GAnnotationsUtils.canResolveAnnotation(e)
          : GAnnotationsUtils.canReopenAnnotation(e)) &&
          $('<span/>')
            .addClass('assigned-action-group')
            .append(
              $('<span/>')
                .addClass('icon ' + (AppSettings ? 'gravit-icon-resolve' : 'gravit-icon-resolved'))
                ...
                .attr(
                  ...
                  AppSettings
                    ? GCore.GLocale.get(
                        new GCore.GLocaleKey('GAnnotationPanel', 'text.assign-resolve')
                      )
                    : GCore.GLocale.get(new GCore.GLocaleKey('GAnnotationPanel', 'text.reopen'))
                )
                .on('click', () => {
  ...
  function B(e) {
    return e.getChildren().filter((e) => e instanceof GCore.GComment && !e.getProperty('rmd'));
  };
  function U(e) {
  ...
  function K(e, t, n) {
    ...
    if (
      ...
      if ((GCollaborationMentionsUtils.beginUpdate(), e.hasMixin(GCore.GAnnotation))) {
        ...
        AppSettings = GSystemDialog
          ? R.call(this, _interopRequireDefault, GSystemDialog, false)
          : M.call(this, _interopRequireDefault, null, false);
      } else {
      ...
      if (
        ...
        for (
          var GAnnotationReplyEditor = e.getFirstChild();
          null !== GAnnotationReplyEditor;
          GAnnotationReplyEditor = GAnnotationReplyEditor.getNext()
        )
        ...
        ((GTools.annotTreeNodeMap[n] = _interopRequireDefault),
          GTools.replyNodes.set(GUserModel, _interopRequireDefault));
      }
  ...
  function V(e) {
    ...
    return (
      n &&
        ...
        t.delete(e),
        ($(this).data('gannotationpanel').annotTreeNodeMap[n.treeNode.id] = null)),
      n
    );
  };
  function H(e, t) {
    if (t)
      !t.getParent() ||
        ...
        GAnnotationsUtils.removeAnnotations(
          ...
          t.getParent(),
          GCore.GLocale.get(
            new GCore.GLocaleKey('GAnnotationPanel', 'text.remove-empty-annotation')
          )
        );
    else {
  ...
  function X(e, t) {
    ...
    if (
      ...
      for (
        var GCollaborationMentionsUtils = 0, AppSettings = e.values.length;
        GCollaborationMentionsUtils < AppSettings;
        GCollaborationMentionsUtils++
      )
        GTools[e.properties[GCollaborationMentionsUtils]] = e.values[GCollaborationMentionsUtils];
    if (e && e.node) {
      if (e.custom) return;
      if (e.node.hasMixin(GCore.GAnnotation) && e.node.isPropertiesIgnorable(e.properties)) return;
    }
    ...
    GInvalidationOptions &&
      ...
      (require.scheduledUpdate = setTimeout(() => {
        ...
        GAnnotationsUtils.updateAndReturnCloudAnnotationsForDocument(
          GInvalidationOptions,
          GCore.GNode.store(require, {
            recordedTransaction: _interopRequireDefault,
            recordedProperties: GTools,
          })
        )
          .then((e) => {
  ...
  function J(e) {
    q($(this).data('gannotationpanel'), e.node) &&
      (z.call(this, e.node),
      e.node.findParent((e) => e instanceof GCore.GAnnotationsList) && X.call(this));
  };
  function Z(e) {
        ...
        setTimeout(() => {
          ...
          (undefined !== require && false !== require) !== e.node.getProperty('rsv') &&
            ...
            e.node.addComment(
              ...
              t.avatar,
              t.getAccountName(),
              e.node.getProperty('rsv') ? GCore.GComment.Type.Close : GCore.GComment.Type.Open
            ));
        }),
        (e.node instanceof GCore.GAnnotationsList ||
          e.node.findParent((e) => e instanceof GCore.GAnnotationsList)) &&
          (e.node.hasMixin(GCore.GAnnotation) && e.properties.indexOf('uid') >= 0 && j(e.node),
          X.call(this, e)),
        e.properties.includes('rmd') && e.node.getProperty('rmd')
          ? (e.node.hasMixin(GCore.GAnnotation) && W(this, e.node), ne.call(this), te.call(this))
  ...
  function ee(e) {
    ...
    if (q(n, e.node)) {
      ...
      if (e.node instanceof GCore.GComment || e.node.hasMixin(GCore.GAnnotation))
        if (
          ...
          if (
            ((GAnnotationsUtils &&
              GCollaborationMentionsUtils &&
              GAnnotationsUtils !== GCollaborationMentionsUtils) ||
              (_interopRequireDefault = true),
            e.node.hasMixin(GCore.GAnnotation) &&
              e.flag === GCore.GNode.Flag.Selected &&
              false === e.set &&
            ...
            if (e.node.isEmptyTextAllowed()) {
              ...
              GSystemDialog.confirm(
                GCore.GLocale.get(
                  new GCore.GLocaleKey('GAnnotationPanel', 'text.confirm-discard-annotation')
                ),
                (t) => {
                  if (t)
                    setTimeout(() => {
      ...
      _interopRequireDefault &&
        oe.requestInvalidation.call(this, GInvalidationOptions.default.NO_CACHE_INVALIDATION);
    }
  };
  function te() {
