...
function (exports, module, require) {
  ...
  function a(e) {
    for (var module = 0; module < i.length; ++module)
      if (i[module].type === e) return i[module].icon;
    return null;
  }
  function r(e) {
    for (var module = 0; module < i.length; ++module)
          ...
          (i = [
            {
              ...
              title: GCore.GLocale.get(new GCore.GLocaleKey('GPathBase', 'corner.rounded')),
              icon: 'gravit-icon-corner-rounded',
            },
            {
              type: GCore.GPathBase.CornerType.InverseRounded,
              title: GCore.GLocale.get(new GCore.GLocaleKey('GPathBase', 'corner.inverse-rounded')),
              icon: 'gravit-icon-corner-inverse-rounded',
            },
            {
              type: GCore.GPathBase.CornerType.Bevel,
              title: GCore.GLocale.get(new GCore.GLocaleKey('GPathBase', 'corner.bevel')),
              icon: 'gravit-icon-corner-bevel',
            },
            {
              type: GCore.GPathBase.CornerType.Inset,
              title: GCore.GLocale.get(new GCore.GLocaleKey('GPathBase', 'corner.inset')),
              icon: 'gravit-icon-corner-inset',
            },
            {
              type: GCore.GPathBase.CornerType.Fancy,
              title: GCore.GLocale.get(new GCore.GLocaleKey('GPathBase', 'corner.fancy')),
