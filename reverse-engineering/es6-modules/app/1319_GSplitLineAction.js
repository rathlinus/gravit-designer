...
function (exports, module, require) {
  ...
  class l extends GElementAction {
    ...
    execute() {
      ...
      if (n)
        for (var MenuItemBuilder = 0; MenuItemBuilder < n.length; ++MenuItemBuilder) {
          ...
          GElementAction instanceof GCore.GPath &&
            this._isPathSplittable(GElementAction) &&
            GTools.push(GElementAction);
        }
      if (GTools.length) {
        t.beginTransaction();
        try {
          ...
          for (MenuItemBuilder = 0; MenuItemBuilder < GTools.length; ++MenuItemBuilder)
            l.add(GTools[MenuItemBuilder].getParent());
          try {
            ...
            for (MenuItemBuilder = 0; MenuItemBuilder < GTools.length; ++MenuItemBuilder) {
              ...
              else (d = h.getFirstChild()).hasFlag(GCore.GNode.Flag.Selected) || (f = true);
              var m = d,
                y = m,
              ...
              for (
                ...
                null !== m && (m.hasFlag(GCore.GNode.Flag.Selected) || f) && null !== v;
              ) {
                var b,
                  w = new GCore.GPath(),
                  C = w.getAnchorPoints();
                if (
                  ...
                  ((b = new GCore.GPathBase.AnchorPoint()).deserialize(m.serialize()),
                    (m = b),
                    (_ = false));
                else h.removeChild(m);
                for (
                  C.appendChild(m);
                  null !== d && !d.hasFlag(GCore.GNode.Flag.Selected) && h.getFirstChild();
                )
                  ((v = h.getNextPoint(d)), h.removeChild(d), C.appendChild(d), (d = v));
                if (null !== d && d.hasFlag(GCore.GNode.Flag.Selected) && h.getFirstChild())
                  ((b = new GCore.GPathBase.AnchorPoint()).deserialize(d.serialize()),
                    C.appendChild(b),
                ...
                w.isLine() &&
                  (w.getPaintLayers().getBorderLayers(true).length ||
                    (u.getPaintLayers().getFillLayers(true).length
                      ? w
                          .getPaintLayers()
                          .appendChild(new GCore.GStylable.BorderPaintLayer(GCore.GRGBColor.BLACK))
    ...
    _isPathSplittable(e) {
      ...
      if (GCore && GCore.length)
        for (
          ...
          !t && CollaborationMergeUtils < GCore.length;
    ...
    static ID = 'modify.split-line';

    static TITLE = new GCore.GLocaleKey('GSplitLineAction', 'title');
