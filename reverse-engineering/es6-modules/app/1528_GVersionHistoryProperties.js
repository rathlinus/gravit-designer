...
function (exports, module, require) {
  ...
  class y extends GProperties.default {
    constructor() {
      super();
    }

    _relayoutBindedToActiveDocument = null;
    _panel = null;
    _toolbar = null;
      ...
      (t.addClass('filled'),
        ...
        $('<label/>')
          ...
          .append(
            $('<span />')
              .addClass('pro')
              .text(GCore.GLocale.get(new GCore.GLocaleKey('GVersionHistoryProperties', 'title')))
          )
          .appendTo(t),
        $('<button/>')
        ...
        (this._versionsContainer = $('<div/>')
          ...
          .append([
            $('<div/>')
              ...
              .text(
                GCore.GLocale.get(
                  new GCore.GLocaleKey('GVersionHistoryProperties', 'text.title-manual-save')
                )
              )
              .gRichTooltip(
        ...
        (this._autoSaveContainer = $('<div/>')
          ...
          .append([
            $('<div/>')
              ...
              .text(
                GCore.GLocale.get(
                  new GCore.GLocaleKey('GVersionHistoryProperties', 'text.title-auto-save')
                )
              )
              .gRichTooltip(
                GRichTooltipConfig.default.from({
    ...
    _updateVersionHistory(e) {
      (this._versionsContainer.find('.content').empty(),
        AppSettings.AUTO_SAVE_ENABLED && this._autoSaveContainer.find('.content').empty(),
          ...
          for (let e = 0, t = require ? require.length : 0; e < t; e++)
            require[e]
              ? _interopRequireDefault.push(
                  AppSettings.gApi.getFile(this._fileId, false, require[e].versionId, 't')
                )
              : _interopRequireDefault.push(null);
          return Promise.all(_interopRequireDefault).then((e) => {
            let require = [];
            for (
              let _interopRequireDefault = 0, GCore = t.length;
        ...
        require.push(
          AppSettings.gApi.listAutoSaves(this._fileId).then((e) => {
            let t = [],
              require = [];
            for (
              let t = 0, _interopRequireDefault = e.versions.length;
              t < _interopRequireDefault;
              t++
            )
              require.push(
                e.versions_t && e.versions_t[t]
                  ? AppSettings.gApi.getAutoSaveThumbnail(this._fileId, e.versions_t[t].versionId)
                  : null
              );
            return Promise.all(require).then((n) => {
              for (
                let _interopRequireDefault = 0, GCore = e.versions.length;
                _interopRequireDefault < GCore;
                _interopRequireDefault++
              ) {
                let GCore = {
                  ...
                  thumbnail: {
                    name: gDesigner.getWindows().getActiveWindow().getTitle(),
                    url_t:
                      (n && n[_interopRequireDefault] && n[_interopRequireDefault].url) ||
                      'assets/icon/versus.svg',
                  },
                  autosave: true,
                };
                t.push(GCore);
              ...
              .append(
                $('<div />')
                  ...
                  .text(
                    GProperties.versionId === GDocumentStatusEvent.version.versionId
                      ? GCore.GLocale.get(
                          new GCore.GLocaleKey('GVersionHistoryProperties', 'text.current-version')
                        )
                      : GCore.GLocale.get(
                          new GCore.GLocaleKey('GVersionHistoryProperties', 'text.version')
                ...
                $('<div />')
                  ...
                  .append(
                    $('<div />')
                      ...
                      .text(
                        GCore.GLocale.get(
                          new GCore.GLocaleKey('GVersionHistoryProperties', 'preview')
                        )
                      )
                      .on('click', function (e) {
                        (e.stopPropagation(),
                  ...
                  .append(
                    $('<div />')
                      ...
                      .text(
                        GCore.GLocale.get(
                          new GCore.GLocaleKey('GVersionHistoryProperties', 'restore')
                        )
                      )
                      .on('click', function (e) {
                        (e.stopPropagation(),
    ...
    _updatePanelHeight() {
      let exports =
        parseInt(this._panel.closest('.sidebar-inspector').outerHeight(), 10) -
        parseInt(this._toolbar.outerHeight(), 10);
      this._panel.css('height', exports);
    }

    _documentStatusEventHandler(e) {
      let { status: module } = e;
    ...
    async _showPreview(e, t, n, _interopRequireDefault) {
      ...
      if (this._previewOverlay) {
        ...
        ((this._buttonsToolbar = $('<div />')
          ...
          .css({
            ...
            background:
              _interopRequireDefault.css('background') ||
              _interopRequireDefault.css('background-color'),
          ...
          .append(
            $('<button />')
              ...
              .text(
                GCore.GLocale.get(new GCore.GLocaleKey('GVersionHistoryProperties', 'edit-version'))
              )
              .on(
                'click',
                function () {
          ...
          .append(
            $('<button />')
              ...
              .text(
                GCore.GLocale.get(
                  new GCore.GLocaleKey('GVersionHistoryProperties', 'close-preview')
                )
              )
              .on(
                'click',
    ...
    _closePreview() {
      (this._enabledInputs &&
        ...
        this._previousDoc &&
          (gDesigner.openDocument(this._previousDoc.getStorageItem()), (this._previousDoc = null)),
        this._previewDoc &&
          (this._previewDoc.removeEventListener(
            GDocumentStatusEvent.default,
            this._documentStatusEventHandler,
    ...
    static ID = 'version-history';

    static TITLE = new GCore.GLocaleKey('GVersionHistoryProperties', 'title');
