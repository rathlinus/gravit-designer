  ...
  class v extends GProperties {
    ...
    init(e, t) {
        ...
        if ('evenodd' === e)
          return $('<select></select>')
            ...
            .append(
              $('<option></option>')
                .attr('value', '1')
                .text(GCore.GLocale.get(new GCore.GLocaleKey('GCommonNames', 'evenodd.even-odd')))
            )
            .on('change', function () {
              (gDesigner.stats(
      ...
      (this._toolbar.addClass('list-toolbar fill-toolbar'),
        $('<label></label>')
          .text(GCore.GLocale.get(new GCore.GLocaleKey('GFillPaintLayerProperties', 'title')))
          .appendTo(this._toolbar),
        $('<button></button>')
          .attr(
            'data-title',
            GCore.GLocale.get(
              new GCore.GLocaleKey('GFillPaintLayerProperties', 'action.advanced-settings')
            )
          )
        ...
        $('<button></button>')
          ...
          .attr(
            'data-title',
            GCore.GLocale.get(
              new GCore.GLocaleKey('GFillPaintLayerProperties', 'action.remove-selected')
            )
          )
            ...
            function (e) {
              var t = this._panel.find('.copy-info-overlay').eq(0),
                ...
                i = $('<span/>')
                  ...
                  .text(
                    GCore.GLocale.get(
                      new GCore.GLocaleKey('GFillPaintLayerProperties', 'text.copy-fill')
                    )
                  );
              (t && t.remove(),
      ...
      if (this._document) {
        ...
        try {
          ...
          (_interopRequireDefault.commitTransaction(
            n ||
              GCore.GLocale.get(new GCore.GLocaleKey('GFillPaintLayerProperties', 'action.modify'))
          ),
          ...
          return '_bl' === e
            ? $('<select></select>')
                ...
                .on('change', function (e) {
                  (gDesigner.stats('fill_change_blend-mode', $(e.target).val()),
                    b(['_bl'], [$(e.target).val()]));
                })
            : '_op' === e
              ? $('<input>')
                ...
                content: $('<input>')
                  ...
                  .on('change', function (e) {
                    (gDesigner.stats('fill_change_opacity'),
                      b(
                        ['_vs', '_op'],
                        [true, GCore.GLength.parseEquationValue($(this).gInputBox('value')) / 100]
                      ));
                  })
                  .gInputBox({
                    minValue: 0,
