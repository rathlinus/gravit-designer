...
function (exports, module, require) {
  ...
  class c extends GElementAction {
        ...
        var d = $('<div></div>')
          .append(
            $('<div>')
              ...
              .on('change', function (e) {
                if (GEditor) {
                  var t = Number(GEditor.stringToPoint($(this).gInputSlider('value')).toFixed(0));
          ...
          .append(
            $('<span>')
              ...
              .append(
                $('<label>').html(
                  GCore.GLocale.get(new GCore.GLocaleKey('GSimplifyAction', 'text.tolerance'))
                )
              )
          );
        GSystemDialog.prompt(
          GCore.GLocale.get(new GCore.GLocaleKey('GSimplifyAction', 'text.simplification')),
          (e) => {
            if (e) {
              var n = parseFloat(d.find('.g-input-slider').gInputSlider('value'));
              if (isNaN(n) || !isFinite(n) || GCore.GMath.isEqualEps(n, 0))
                GSystemDialog.alert(
                  GCore.GLocale.get(new GCore.GLocaleKey('GSimplifyAction', 'text.invalid-value'))
                );
              else {
                t.beginTransaction();
                try {
                  for (
                    var GEditor = [], GElementAction = new Set(), c = 0;
                    c < MenuItemBuilder.length;
                    ++c
                  ) {
    ...
    static ID = 'modify.simplify';

    static TITLE = new GCore.GLocaleKey('GSimplifyAction', 'title');
