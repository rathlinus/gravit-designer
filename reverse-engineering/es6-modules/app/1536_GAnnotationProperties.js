...
function (exports, module, require) {
  ...
  class p extends GProperties {
    constructor(e, t, n, _interopRequireDefault, GTools, GCore, GAnnotationsUtils, GProperties) {
      super();
      ((this._elements = []),
    ...
    init(e, t) {
      ...
      var n = function (e) {
          var t = this;
          if (e === p.PropertySet.BorderLayer || e === p.PropertySet.FillLayer) {
            let n,
            ...
            return (
              GTools.find('span.preview.g-button').attr(
                'data-title',
                GCore.GLocale.get(new GCore.GLocaleKey('GAnnotationProperties', this._tooltips[e]))
              ),
              GTools.find('div.eyedropper').attr(
                'data-title',
                GCore.GLocale.get(
                  new GCore.GLocaleKey('GAnnotationProperties', this._tooltips[e + 'dropper'])
                )
              ),
              (n =
                e === p.PropertySet.BorderLayer
              ...
              GTools.append($('<span/>').addClass('layer-title').text(GCore.GLocale.get(n))),
              GTools
            );
          }
          ...
          if (e === p.PropertySet.BorderHeadMarker || e === p.PropertySet.BorderTailMarker) {
            const n = (n) => {
                ...
                .append(
                  $('<span>').text(
                    e === p.PropertySet.BorderTailMarker
                      ? GCore.GLocale.get(
                          new GCore.GLocaleKey('GAnnotationProperties', 'text.end-arrow')
                        )
                      : GCore.GLocale.get(
                          new GCore.GLocaleKey('GAnnotationProperties', 'text.start-arrow')
                        )
                  )
                )
            );
        ...
        _interopRequireDefault = [];
      this._availableProperties.indexOf(p.PropertySet.BorderHeadMarker) >= 0 &&
        this._availableProperties.indexOf(p.PropertySet.BorderTailMarker) >= 0 &&
      ...
      for (
        var GAnnotationsUtils = 0;
        GAnnotationsUtils < this._availableProperties.length;
        GAnnotationsUtils++
      ) {
      ...
      ($('<div></div>').gPropertyRow({ columns: _interopRequireDefault }).appendTo(this._panel),
        ...
        (this._toolbarButton = $('<button>')
          ...
          .attr(
            'data-title',
            GCore.GLocale.get(new GCore.GLocaleKey('GAnnotationProperties', this._toolbarTooltip))
          )
          .on('click', () => {
            if (
    ...
    _updateTouchComponents(e) {
      const module = this._panel.find('.custom-checkbox-mode');
      gDesigner.isTouchEnabled() ? module.gCheckboxSlider() : module.gCheckboxSlider('unmount');
    }
      ...
      if (
        ...
        if (
          ...
          for (
            ...
            ((GTools instanceof GCore.GStyle && n && n instanceof this._propertyTool) ||
              (GTools instanceof this._propertyClass && GTools.hasMixin(GCore.GAnnotation))) &&
    ...
    async _handleReviewUpdate(e) {
      (this._updateToolbar(), this._elements.length && this._updateProperties());
    }

    async _stateChangedEvent(e) {
      (this._updateToolbar(),
        this._document && e.document === this._document && this._updateProperties());
    }

    _getAppManager() {
      return (
        this._appManager || (this._appManager = gDesigner.getApplicationManager()),
        this._appManager
      );
    }
    ...
    _recordPendingUpdateForSelection(e, t, n, _interopRequireDefault, GTools) {
    ...
    _cleanPendingUpdateForSelection(e) {
      this._pendingUpdates.delete(e);
    }

    _applyPendingUpdateForSelection() {
      let exports = arguments.length > 0 && undefined !== arguments[0] ? arguments[0] : null;
      function module(e) {
        const module = this._pendingUpdates.get(e);
        module &&
          this._assignProperties(module.props, module.values, module.title, false, module.target);
      }
      exports
    ...
    _updateProperties() {
      if (!this._elements || !this._elements.length)
        return void console.warn('GAnnotationProperties: empty _elements array');
      ...
      if (this._availableProperties.indexOf(p.PropertySet.FillLayer) >= 0) {
        ...
        (this._panel
          ...
          .gPatternChooser(
            'setPattern',
            _interopRequireDefault
              ? _interopRequireDefault.getProperty('_pt', false, false, true)
              : null
          )
          .gPatternChooser(
            'value',
          ...
          .gPatternChooser(
            'opacity',
            _interopRequireDefault
              ? _interopRequireDefault.getProperty('_op', false, false, true)
              : null
          ),
          module && require
            ? this._panel
    ...
    _assignProperty(e, t, n, _interopRequireDefault, GTools, GCore) {
      this._assignProperties([e], [t], n, _interopRequireDefault, GTools, GCore);
    }

    _assignProperties(e, t, n, _interopRequireDefault, GTools, GAnnotationsUtils) {
      if (this._document) {
            ...
            (GTools === p.PropertyTarget.FillLayer
              ? ((GFileReviewFlowManager =
                ...
                (GCollaborationEvent = $.extend(
                  {
                    fillLayerIndex:
                      GFileReviewFlowManager.getParent().getIndexOfChild(GFileReviewFlowManager),
                  },
