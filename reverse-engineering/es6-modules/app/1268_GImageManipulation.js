    ...
    (module.cropImage = function (e, t) {
      ...
      t &&
        GTools.GEditor.tryRunTransaction(
          ...
          GCore.GLocale.get(new GCore.GLocaleKey('GImageProperties', 'action.reset-cropping'))
        );
                      ...
                      if (
                        ...
                        (s.activateTool(GTools.GPointerTool),
                          s.getActiveTool() instanceof GTools.GSelectTool &&
                            s.getActiveTool().setEditMode(GTools.GSelectTool.EditMode.Select));
                        var l = e.getGeometryBBox(),
                          c = new GCore.GTransform().translated(l.getX(), l.getY());
                        (GTools.GEditor.tryRunTransaction(
                          e,
                          () => {
                            e.setProperties(
                          ...
                          GCore.GLocale.get(
                            new GCore.GLocaleKey('GImageProperties', 'action.replace-image')
                          )
                        ),
                          s.getActiveTool() instanceof GTools.GSelectTool &&
                            s.getActiveTool().setEditMode(GTools.GSelectTool.EditMode.Edit));
                      }
                    }),
                      (n.src = t));
                  }
