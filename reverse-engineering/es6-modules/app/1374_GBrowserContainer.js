...
function (exports, module, require) {
  ...
  const DataModule_1482 = require(1482) /* DataModule_1482 */,
    { base64StringToString: S } = require(40);
  class E {
    constructor() {
      ((this._storage = new s()),
    ...
    start() {
      ...
      if (GObject.searchParams) {
        if (GObject.searchParams.get('token') && GObject.searchParams.get('d'))
          ...
          i = new r.OpenFileRequest(r.OpenFileRequest.Type.Document, GObject.searchParams.get('d'));
          ...
          try {
            (e = JSON.parse(S(decodeURIComponent(t))).type) === r.OpenFileRequest.Type.Preset
              ...
              : e === r.OpenFileRequest.Type.Template &&
                (i = new r.OpenFileRequest(r.OpenFileRequest.Type.Template, t));
          } catch (e) {
    ...
    openStorageFile(e, t, n) {
      E.openStorageFile(e, t, n, this._storage);
    }

    handleDeepLinking(e) {
      const module = r.prototype.handleDeepLinking.call(this, e),
        require = [r.DeepLinking.DirectLink, r.DeepLinking.FocusAnnot, r.DeepLinking.CreateShare];
      return (
        module &&
    ...
    static openStorageFile(e, t, n, GObject) {
      var s = { progress: null };
              ...
              t &&
                ...
                (t.presetLayout.template
                  ? ((GGoogleFontsProvider = await gApi
                    ...
                    GGoogleFontsProvider &&
                      n(
                        new GMarketingFileStorageItem(
                          GObject,
                          GGoogleFontsProvider.data,
              ...
              t &&
                ...
                n(
                  new GMarketingFileStorageItem(
                    GObject,
                    i,
                    ''.concat(t.name, '.').concat(GLocaleKey),
                    t.id
                  ),
                  {
                    content: e,
              ...
              if (GGoogleFontsProvider)
                ...
                (gDesigner.getShareManager().isPermissionRequestEnabled() &&
                  GFontsProviderManager.push({
                    label: i.get(new GLocaleKey('GShareManager', 'text.file-request-access')),
                    onclick: (e) => {
                      (gDesigner.stats('permission-dialog_no-access_request-access'),
                        gApi
                          ...
                          .then(() => {
                            (e.gDialog('close'),
                              GSystemDialog.alert(
                                i.get(new GLocaleKey('GShareManager', 'text.sent-request-email'))
                              ));
                          })
                          .catch(() => {
                            GSystemDialog.error(
                              i.get(new GLocaleKey('GShareManager', 'text.cannot-request-access'))
                            );
                          }));
                    },
                  }),
                  ...
                  GSystemDialog.custom({
                    ...
                    title: i.get(
                      new GLocaleKey('GShareManager', 'text.file-can-not-be-accessed-title')
                    ),
                    subtitle: i.get(
                      new GLocaleKey('GShareManager', 'text.file-can-not-be-accessed-info')
                    ),
