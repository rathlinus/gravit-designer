...
function (exports, module, require) {
  ...
  class l extends GCore.GObject {
    constructor(e, t, n) {
      super();
      let AppSettings = arguments.length > 3 && undefined !== arguments[3] ? arguments[3] : {};
      ((this._user = e),
    ...
    _openAccountDeletionConfirmationDialog() {
      let exports = '<span>'
        .concat(
          GCore.GLocale.get(new GCore.GLocaleKey('GAccountPanel', 'text.delete-account-title')),
          '</span>\n    <mark style="min-width: 100px;\n        background: #F23C3C;\n        border-radius: 3px;\n        padding: 3px 30px;\n        color: white;\n        margin-top: 2px;\n        margin-bottom: 2px;\n        display: block;\n        text-align: center;">\n        '
        )
        .concat(
          GCore.GLocale.get(new GCore.GLocaleKey('GAccountPanel', 'text.delete-account-sub-title')),
          '\n    </mark>\n    <ul style="list-style-type: disc; transform: translateX(30px); margin-bottom: 10px;">\n        <li>'
        )
        .concat(
          GCore.GLocale.get(new GCore.GLocaleKey('GAccountPanel', 'text.delete-account-list-1')),
          '</li>\n        <li>'
        )
        .concat(
          GCore.GLocale.get(new GCore.GLocaleKey('GAccountPanel', 'text.delete-account-list-2')),
          '</li>\n        <li>'
        )
        .concat(
          GCore.GLocale.get(new GCore.GLocaleKey('GAccountPanel', 'text.delete-account-list-3')),
          '</li>\n    </ul>\n    <span>'
        )
        .concat(
          GCore.GLocale.get(
            new GCore.GLocaleKey('GAccountPanel', 'text.delete-account-action-cant-undone')
          ),
          '</span>'
        );
        ...
        $('<div></div>')
          ...
          .append(
            $('<button></button>')
              .addClass('highlight')
              .attr('type', 'submit')
              .text(GCore.GLocale.get(new GCore.GLocaleKey('GAccountPanel', 'text.save')))
          )
          .append(
            $('<button></button>')
              .attr('type', 'button')
              .attr('data-property', 'delete-account')
              .text(GCore.GLocale.get(new GCore.GLocaleKey('GAccountPanel', 'text.delete')))
              .on(
                'click',
                (e) => (e.preventDefault(), this._openAccountDeletionConfirmationDialog(), false)
              )
          )
      );
      return (
          ...
          (t &&
            ...
            this._container.find('.cb-link').on('click', (e) => {
              (gDesigner.stats('profile-dialog_account-panel_cleverbridge-link'),
                gContainer.openExternalLink(e, AppSettings.LINKS.CLEVERBRIDGE_SUPPORT_URL));
            }));
        }),
        exports
      );
    ...
    _init() {
        ...
        module = $('<div></div>')
          ...
          .append(
            $('<div></div>')
              ...
              .append(
                $('<span></span>').text(
                  GCore.GLocale.get(new GCore.GLocaleKey('GAccountPanel', 'text.contact'))
                )
              )
          )
          .appendTo(exports),
      ...
      if (!l) {
        let { first: exports, last: module } = ((e) => {
          ...
          return (
            require.trim().length || ((require = AppSettings), (AppSettings = '')),
            { first: require, last: AppSettings }
          );
        })(s);
        ((s = exports), (l = module || ''));
      }
      let c = $('<div></div>').addClass('group-section').appendTo(module);
      (a(GCore.GLocale.get(new GCore.GLocaleKey('GAccountPanel', 'text.first-name')), s, 'name')
        .appendTo(c)
        .find('input')
        .attr('required', true),
        a(
        ...
        a(
          GCore.GLocale.get(new GCore.GLocaleKey('GAccountPanel', 'text.old-password')),
          '',
          'old_password',
          'password'
        ).appendTo(module),
        a(
          GCore.GLocale.get(new GCore.GLocaleKey('GAccountPanel', 'text.new-password')),
          '',
          'new_password',
          'password',
          GCore.GLocale.get(new GCore.GLocaleKey('GPurchasePanel', 'text.placeholder-password'))
    ...
    async _updateUser() {
      this._toggleLoading(true);
      const exports = (e) => {
        let module = GCore.GLocale.get(new GCore.GLocaleKey('GAccountPanel', 'text.error'));
        (e && e.message
          ? (module = e.message)
      ...
      (this._container.find('.input-field > input').each(function () {
        ...
        (('login' !== AppSettings && 'email' !== AppSettings) ||
          require._user[AppSettings] !== exports.val().trim()) &&
        ...
        await gDesigner
          ...
          .then(() =>
            this._messageHandler(
              GCore.GLocale.get(new GCore.GLocaleKey('GAccountPanel', 'text.success')),
              'success'
            )
          )
          .then(() => {
