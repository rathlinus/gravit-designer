...
function (exports, module, require) {
  ...
  let x;
  class S extends CloudException.default {
    constructor() {
      super();
      ...
      (this.addAction({
      title: GCore.GLocale.get(new GCore.GLocaleKey('GGoogleDrive', 'text.add-files')),
      icon: 'gravit-icon-add-files',
      execute: e,
      }),
      this.setDefaultEmptyMessage({
      title: GCore.GLocale.get(new GCore.GLocaleKey('GGoogleDrive', 'text.you-have-not-added')),
      buttons: [
      {
      ...
      .append(
      $('<span/>').text(
      GCore.GLocale.get(new GCore.GLocaleKey('GGoogleDrive', 'text.warning-message'))
      )
      )
      .append(
      $('<div/>')
    ...
    _securityLevel = barrel_purchase_urls.SecurityLevel.Lowest;
    _googlePickerLoaded = false;
    _googleDriveClient = null;
    CURRENT_FOLDER_PROP = 'designer.filespanel.google-drive.current-folder';

    setCorporateStorage(e) {
      if (
        (CloudException.default.prototype.setCorporateStorage.call(this, e),
        S.LAST_TEAM_DRIVE_ID_PROP_NAME)
      ) {
        let t = null;
        (e && ({ id: t } = e), this.saveLastTeamDriveId(S.LAST_TEAM_DRIVE_ID_PROP_NAME, t));
      }
    }
        ...
        (a &&
          GSystemDialog.messageWithInfo({
            mainMessage: GCore.GLocale.get(
              new GCore.GLocaleKey('GGoogleDrive', 'text.selected-files-folder-not-added')
            ),
            ...
            'folder' !== AppError
              ? (require(GDocumentEvent).then((e) => {
                  ...
                  t &&
                    GSystemDialog.messageWithInfo({
                      mainMessage: GCore.GLocale.get(
                        new GCore.GLocaleKey('GGoogleDrive', 'text.selected-file-folder-not-added')
                      ),
                      infoMessage: GCore.GLocale.get(
                        new GCore.GLocaleKey(
                          'GGoogleDrive',
                  ...
                  .then(
                    (e) =>
                      new Promise((t, n) => {
                        ...
                        GSystemDialog.alert(
                          GCore.GLocale.get(
                            new GCore.GLocaleKey('GDocument', 'text.unsupported-file-extension')
                          ),
                          n
                        );
                      })
    ...
    isRootFolder(e) {
      ...
      return 'string' == typeof e ? e === t.id : !e || (e && !e.id) || e === t || e.id === t.id;
    }
      ...
      return new Promise(async (e, t) => {
        try {
          ...
          if (this._settings.corporate) require = await gDesigner.getUser();
            ...
            if (!gContainer.getGoogleAPI().isLoaded()) return t('Google Drive Client not loaded!');
      ...
      return new Promise(async (e, t) => {
        ...
        try {
          if (this._apiKey && this._clientId)
            ((a = this._clientId),
              (_interopRequireDefault = this._apiKey),
              (GDocumentEvent = this._appId));
          else {
          ...
          return t(GCore.GLocale.get(new GCore.GLocaleKey('GCommonNames', 'text.loading-failed')));
        }
        gContainer
          .getGoogleAPI()
          .init({
      ...
      return new Promise(async (e, t) =>
        gContainer.getGoogleAPI().isLoaded()
          ? (await this.isSignedIn())
            ? ((this._googleDriveClient = await this._buildGoogleClient()), e())
            : gContainer
                .getGoogleAPI()
                .signIn()
                .then(async () => {
    ...
    _buildSearchQuery(e) {
      ...
      return (
        _interopRequireDefault
          ...
          : GCore &&
            (module.group((e) => {
              (this.getSupportedExtensions().forEach((t) => e.or('fileExtension', '=', t)),
                this.getSupportedMIMETypes().forEach((t) => e.or('mimeType', '=', t)));
            }),
    ...
    getFolders() {
      ...
      return (t && (e[t] = { folder: { name: null, id: null, parent: null } }), e);
    }
      ...
      return new Promise(async (_interopRequireDefault, GCore) => {
        try {
          var a = {
              ...
              pageToken: (e.hasValue('nextPageToken') && e.nextPageToken) || '',
            },
    ...
    async _convertToCloudItems(e) {
      return (e = e instanceof Array ? e : [e]).map((e) => GGoogleDriveItem.convertToCloudItem(e));
    }
    ...
    async _createStorageItem(e, t, n) {
      ...
      return new Promise(async (AppError, d) => {
        const DataModule_1301 = function (e, t, n) {
          ...
          const _interopRequireDefault = gDesigner.getHeader(),
            ...
            GDocumentEvent = _interopRequireDefault.getWindowTab(
              gDesigner.getWindows().getWindow(n)
            );
          (a && GDocumentEvent
        ...
        try {
          ...
          const a = (this.findFileFormatByExtension(require) || this.getDefaultFileFormat()).mime,
          ...
          return GGoogleDriveItem.deserializeData(_)
            .then(async () => {
                      ...
                      gDesigner.hasEventListeners(GDocumentEvent.default) &&
                        (gDesigner.trigger(
                          new GDocumentEvent.default(
                            GDocumentEvent.default.Type.Modified,
                            GGoogleDriveItem
                          )
                        ),
                        gDesigner.trigger(
                          new GDocumentEvent.default(
                            GDocumentEvent.default.Type.Activated,
                            GGoogleDriveItem
                          )
                        )),
    ...
    fetchRecentFiles() {
      return this._fetchFiles(null, 0, '-viewed', '*', this._currentRecentFilesPage).then(
        (e) => ((this._currentRecentFilesPage = e), e.getItems())
      );
    }

    async fetchFiles(e, t, n) {
      ...
      return (
        ...
        new Promise(async (_interopRequireDefault, GCore) => {
          try {
            ...
            a = module.getSize() + a.length > n ? a.slice(0, Math.max(n - module.getSize(), 0)) : a;
            var s = await this._convertToCloudItems(a);
            return (
              module.update({ nextPageToken: GDocumentEvent, items: s }),
              module.getSize() < n &&
                GDocumentEvent &&
                (module = await this.getStorages(exports, module)),
    ...
    async generatePreviousSelectedFolderPath() {
      const exports = this.getCurrentFolder();
      if (this.isRootFolder(exports))
        return ((this.PREVIOUS_SELECTED_FOLDER_PATH = []), this.PREVIOUS_SELECTED_FOLDER_PATH);
      ...
      if (module && module !== this._rootFolderID) {
        ...
        for (; e; )
          (this.PREVIOUS_SELECTED_FOLDER_PATH.push(e.id),
            (module = e.getParentId() ? e.getParentId() : e.parents ? e.parents[0] : null),
            (e =
    ...
    static LAST_TEAM_DRIVE_ID_PROP_NAME = (b &&
        b.cloudOptions &&
        (b.cloudOptions.find((e) => 'googledrive' === e.type) || {}).lastTeamDrivePropName) ||
      null;
