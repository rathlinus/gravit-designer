...
function (exports, module, require) {
  ...
  var GCore = require(1) /* GCore */,
    ...
    REARRANGE_TAB_ = require(1517);
  const barrel_panels = require(257);
  ...
  const GSettingChangedEvent = require(135) /* GSettingChangedEvent */,
    ...
    M = [
                        ...
                        if (GMenu2.supportsSaveCollisionFlow()) {
                          ...
                          if (GMenu2.requiresOverwriteCollisionHandling())
                            for (
                              ...
                              GMenuBar = ''
                                .concat(GDocumentStatusEvent, ' (')
                                .concat(++_interopRequireDefault, ')');
                        }
                        (GMenuBar || (GMenuBar = GDocumentStatusEvent),
                          await GMenu2.renameItem(e, GMenuBar));
                      } catch (e) {
                        return (
      ...
      },
      {
        ...
        id: () =>
          ''
            .concat(GGravitCloudAction.default.ID, '.')
            .concat(GGravitCloudAction.default.Actions.SaveAs),
        needsAction: true,
        requiresPro: false,
      },
      { separator: true },
        ...
        callback: function (e) {
          const module = e.getDocument(),
            ...
            _interopRequireDefault = () => {
              const e = module.getScene(),
                require = new GDocument.default(e.clone(null, gDesigner.getWorkspace()));
          ...
          require && require instanceof x.default.Item
            ? (() => {
                ...
                (GCore.setCurrentFolder(GCloudStorageItem.default.from({ id: e })),
      ...
      { separator: true },
      {
        ...
        callback: function (e) {
          ...
          GSystemDialog.default.confirm(
            GCore.GLocale.get(
              new GCore.GLocaleKey('GHeader', 'text.close-other-tabs-confirmation')
            ),
            (e) => {
              e && N(module);
            },
      ...
      },
      {
        ...
        callback: function () {
          GSystemDialog.default.confirm(
            GCore.GLocale.get(new GCore.GLocaleKey('GHeader', 'text.close-all-tabs-confirmation')),
            (e) => {
  ...
  function N(e) {
    ...
    for (
      let require = 0, _interopRequireDefault = e.length;
  ...
  class B {
        ...
        AppSettings.LICENSE.UPGRADEABLE &&
          ...
          $('<div></div>')
            ...
            .append(
              $('<span></span>').text(
                GCore.GLocale.get(
                  new GCore.GLocaleKey('GCommonNames', 'text.try-out-coreldrawpp-pro')
                )
              )
            )
    ...
    _updateViewBasedOnPermissions() {
      const exports = gDesigner.getApplicationManager().isDocumentTabManagementEnabled();
    ...
    _documentEvent(e) {
      ...
      if (
            ...
            if (_interopRequireDefault.data('window') === GEditor[s])
              return (
                _interopRequireDefault
                  .find('.title')
                  .find('.cover')
                  .html(GEditor[s].getTitleWithExtension() + (e.document.isModified() ? '*' : '')),
                _interopRequireDefault
                  .find('.title')
                  .find('input')
                  .val(GEditor[s].getTitle() + (e.document.isModified() ? '*' : '')),
                false
              );
          });
        (e.type !== GDocumentEvent.default.Type.Modified &&
        ...
        e.type === GDocumentEvent.default.Type.AutoSaveSynchronizing
          ? (t.isSynchronizing() || e.type === GDocumentEvent.default.Type.AutoSaveSynchronizing) &&
            (this.updateWindowIcon(n, true, true, t),
            this._updateSyncStatus(
              n,
              GCore.GLocale.get(new GCore.GLocaleKey('GCommonNames', 'text.synchronizing')) + '...',
              true
            ))
          : e.type === GDocumentEvent.default.Type.SynchronismUpdateFailed ||
            ...
            ? this._updateSyncStatus(
                n,
                GCore.GLocale.get(new GCore.GLocaleKey('GCommonNames', 'text.failed-to-synch')),
                false,
                false,
                true,
                t.isCloudFile()
            ...
            'yes' !== $(this).attr('has-been-clicked') &&
              ...
              gDesigner.getUser().then((t) => {
                (t && !gDesigner.isAnonymous()
                  ? e.gUserLogin()
                  : GCloudStorage.default.performLogin(),
      ...
      ($('.login').css('display', e && e.isAnonymous() ? 'none' : ''),
        $('.login .username')
          ...
          .text(
            e
              ? e.getFullUserName()
              : GCore.GLocale.get(new GCore.GLocaleKey('GCommonNames', 'text.cloud-login'))
          ),
        e
    ...
    _removeWindowTab(e) {
      this._windows.find('.tab').each(function (t, n) {
        var _interopRequireDefault = $(n);
        if (_interopRequireDefault.data('window') === e)
          return (_interopRequireDefault.remove(), false);
      });
    }

    _updateActiveWindowTab() {
    ...
    updateWindowIcon(e, t, n, _interopRequireDefault) {
    ...
    _updateSyncStatus(e, t, n, _interopRequireDefault, GCore, GEditor) {
      var AppSettings = $(e).find('.sync-status');
      const s = '.' + barrel_panels['header-cloud'];
      if (
        ($(e).find(s).length > 0 &&
          (n ? $(e).find(s).addClass('animated') : $(e).find(s).removeClass('animated')),
        _interopRequireDefault && $(e).find(s).length > 0 && $(e).find(s).addClass('animated'),
        (AppSettings && 0 !== AppSettings.length) ||
          $('<span/>').addClass('sync-status').insertAfter($(e).find('.close')),
    ...
    getWindowTab(e) {
      for (
        var module = this._windows.find('.tabs').find('.tab'),
          require = null,
      ...
      return (
        M.map((GEditor) => {
          ...
          (GMenuBar
            ...
            GDocumentStatusEvent &&
              ('function' == typeof GDocumentStatusEvent
                ? s.setIcon(GDocumentStatusEvent())
                : s.setIcon(GDocumentStatusEvent)),
