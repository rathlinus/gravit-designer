...
function (exports, module, require) {
  ...
  class l extends GProperties {
    ...
    _panel = null;
    _copiesAndApply = null;
    _copiesAndApplyTouch = null;
    _document = null;
    _elements = null;
    _scaleKeepRatio = false;
    _preserveScaleX = 100;
    _preserveScaleY = 100;
    ...
    init(e, t) {
      ...
      ((this._advancedTransformPanel = $('<div></div>').css('width', '180px').gOverlay({
        ...
        $('<div></div>')
          ...
          .appendTo(this._advancedTransformPanel),
        $('<label></label>')
          .text(GCore.GLocale.get(new GCore.GLocaleKey('GTransformProperties', 'title')))
          .appendTo(t),
        $('<button></button>')
          ...
          .attr(
            'data-title',
            GCore.GLocale.get(
              new GCore.GLocaleKey('GTransformProperties', 'text.advanced-transform-settings')
            )
          )
      ...
      const GTools = a.GRichTooltipConfig.from({
          ...
          description: GCore.GLocale.get(
            new GCore.GLocaleKey('GTransformProperties', 'text.move-tooltip-description')
          ),
          middle: false,
          learnMore: '',
        }),
        GProperties = a.GRichTooltipConfig.from({
          ...
          description: GCore.GLocale.get(
            new GCore.GLocaleKey('GTransformProperties', 'text.scale-tooltip-description')
          ),
          middle: false,
          learnMore: '',
        }),
        s = a.GRichTooltipConfig.from({
          ...
          description: GCore.GLocale.get(
            new GCore.GLocaleKey('GTransformProperties', 'text.rotate-tooltip-description')
          ),
          middle: false,
          learnMore: '',
        }),
        l = a.GRichTooltipConfig.from({
          ...
          description: GCore.GLocale.get(
            new GCore.GLocaleKey('GTransformProperties', 'text.rotate-axis-tooltip-description')
          ),
          middle: false,
          learnMore: '',
        }),
        c = a.GRichTooltipConfig.from({
          ...
          description: GCore.GLocale.get(
            new GCore.GLocaleKey('GTransformProperties', 'text.skew-tooltip-description')
          ),
          middle: false,
          learnMore: '',
        }),
        d = a.GRichTooltipConfig.from({
          ...
          description: GCore.GLocale.get(
            new GCore.GLocaleKey('GTransformProperties', 'text.copies-tooltip-description')
          ),
          middle: false,
          learnMore: '',
        }),
      ...
      ($('<div></div>')
        .gPropertyRow({
          label: GCore.GLocale.get(new GCore.GLocaleKey('GCommonNames', 'action.move')),
          columns: [
            {
              width: '44%',
              content: $('<div/>')
        ...
        $('<div></div>')
          .gPropertyRow({
            label: GCore.GLocale.get(new GCore.GLocaleKey('GTransformProperties', 'text.scale')),
            columns: [
              {
                width: '44%',
                content: $('<div/>')
                    ...
                    $('<input/>')
                      ...
                      .on('change', (e) => {
                        ...
                        if (this._scaleKeepRatio) {
                          ...
                          ((GCore *= n), GTools.gInputBox('value', parseFloat(GCore).toFixed(1)));
                        }
                ...
                content: $('<span></span>')
                  ...
                  .on('click', function (e) {
                    ...
                    'yes' === t.attr('data-ratio')
                      ? (t
                          .attr('data-ratio', 'no')
                          .attr('class', 'gravit-icon-unlinked transform-scale-link'),
                        (n._scaleKeepRatio = false))
                      : (t
                          .attr('data-ratio', 'yes')
                          .attr('class', 'gravit-icon-linked transform-scale-link'),
        ...
        $('<div></div>')
          .gPropertyRow({
            label: GCore.GLocale.get(new GCore.GLocaleKey('GCommonNames', 'action.rotate')),
            columns: [
              {
                width: '44%',
                content: $('<div/>')
                ...
                content: $('<div/>')
                  .append(
                    $('<input/>')
                      ...
                      .on('change', (e) =>
                        gDesigner.stats('transformproperties_change_rotate-down')
                      )
                      .gInputBox({ postfix: '°' })
                      .gInputBox('value', '0')
                  )
        ...
        $('<div></div>')
          .gPropertyRow({
            label: GCore.GLocale.get(new GCore.GLocaleKey('GTransformProperties', 'text.skew')),
            columns: [
              {
                width: '44%',
                content: $('<div/>')
      ...
      (((e) => {
        ($('<div></div>')
          .gPropertyRow({
            label: GCore.GLocale.get(new GCore.GLocaleKey('GTransformProperties', 'text.copies')),
            columns: [
              {
                width: '44%',
                content: $('<input/>')
              ...
              columns: [
                {
                  ...
                  content: $('<button></button>')
                    .text(GCore.GLocale.get(new GCore.GLocaleKey('GCommonNames', 'action.apply')))
                ...
                columns: [
                  {
                    ...
                    content: $('<button></button>')
                      .text(
                        GCore.GLocale.get(
                          new GCore.GLocaleKey('GDimensionProperties', 'text.transform-apply')
                        )
                      )
                      .addClass('transform-button')
                      .on('click', this._applyTransformation.bind(this)),
    ...
    update(e, t) {
      if (
        (this._document &&
          (gDesigner.removeEventListener(s, this._settingChanged), (this._document = null)),
    ...
    _updateDisplayValues() {
      ...
      ((t = isNaN(t) || t <= 0 || !t ? 0 : t),
        e.gInputBox(
          'value',
          GCore.GUtil.formatNumber(t, this._document.getScene().getOptimalDecimalsCount())
        ),
    ...
    _applyTransformation() {
      ...
      var e = this._document.getScene(),
        ...
        d =
          GCore.GMath.toRadians(
            parseFloat(this._panel.find('[data-property="rotate"]').gInputBox('value'))
          ) || 0,
        u =
          GCore.GMath.toRadians(
            parseFloat(this._panel.find('[data-property="skew-x"]').gInputBox('value'))
          ) || 0,
        p =
          GCore.GMath.toRadians(
            parseFloat(this._panel.find('[data-property="skew-y"]').gInputBox('value'))
          ) || 0,
        g =
          parseFloat(
            parseFloat(this._panel.find('[data-property="reflect"]').gInputBox('value'))
          ) || 0;
      ...
      GTools.GEditor.tryRunTransaction(
          ...
          for (
            ...
            if (n > 0)
              for (var c = s.getParent(), d = s.getNext() ? s.getNext() : null, u = 0; u < n; ++u) {
                ...
                (c.insertChild(p, d),
                  u == n - 1 &&
                    (p.setFlag(GCore.GNode.Flag.Selected), s.removeFlag(GCore.GNode.Flag.Selected)),
                  l.push(p));
              }
            e.push(l);
          }
        ...
        GCore.GLocale.get(
          new GCore.GLocaleKey('GTransformProperties', 'action.apply-transformation')
        )
      );
    }
