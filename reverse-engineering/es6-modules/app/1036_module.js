  ...
  module.GDefaultGoogleAPI = new (class extends a {
      ...
      return (
        ...
        new Promise(async (e, t) => {
          try {
            ...
            t(GCore.GLocale.get(new GCore.GLocaleKey('GCommonNames', 'text.loading-failed')));
          }
        })
      );
    }
    async isSignedIn() {
      if (this.isLoaded()) {
        ...
        return fetch(
          'https://www.googleapis.com/oauth2/v3/tokeninfo?access_token='.concat(e && e.access_token)
        )
          .then((e) => e.json())
          .then((e) => !!e.email)
          .catch(() => false);
    ...
    async getBasicProfile() {
      ...
      return fetch(
        ...
        .catch(() =>
          reject(GCore.GLocale.get(new GCore.GLocaleKey('GCommonNames', 'text.loading-failed')))
        );
    }
