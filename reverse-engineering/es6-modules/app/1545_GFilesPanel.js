...
function (exports, module, require) {
  ...
  class T extends GCore.GObject {
    ...
    async _init(e) {
      let {
        parentComponent: module,
        nativeCloud: require,
        initCallback: _interopRequireDefault,
      } = e;
      return (
      ...
      return (async function (n) {
        ...
        (await t.updateCloudSettings(),
          e
            ...
            : (n && (_interopRequireDefault = t.getCloudSettingsById(n)),
              _interopRequireDefault || (_interopRequireDefault = E)));
        try {
      ...
      return gContainer
        ...
        .then(async function (n) {
          ...
          (_interopRequireDefault > -1 && GCore.splice(_interopRequireDefault, 1),
            GCore.length > 0
              ? gContainer.setProperty(t.accountSettingsKey, t._settingsToString(GCore))
        ...
        if (
          ...
          return (
            ...
            void GSystemDialog.default.alert(
              GCore.GLocale.get(new GCore.GLocaleKey('GFilesPanel', 'text.error-creating-folder'))
            )
          );
    ...
    handleDelete() {
      ...
      return (
        GSystemDialog.default.confirm(
          GCore.GLocale.get(new GCore.GLocaleKey('GFilesPanel', 'text.delete-confirm')),
          function (t) {
            t && e.deleteSelection();
          }.bind(this),
          null,
          GCore.GLocale.get(new GCore.GLocaleKey('GFilesPanel', 'action.delete-button')),
          false,
          true,
          true
        ),
    ...
    async handleSave(e, t) {
      ...
      const _interopRequireDefault = (e) =>
        new Promise((t) => {
          GSystemDialog.default.confirm(
            GCore.GLocale.get(
              new GCore.GLocaleKey('GFilesPanel', 'text.file-already-exists-on-current-location')
      ...
      if (
        ...
        try {
          if (this.drive.supportsSaveCollisionFlow()) {
            if (
              (await this.drive.fileExists(e, t, this.drive.getCurrentFolder())) &&
          ...
          if (e && e.badName)
            return (this.view.toggleLoading(false), void GSystemDialog.default.alert(e.message));
          this.getDocumentToSave() &&
            gDesigner.trigger(
              new GDocumentEvent(
                GDocumentEvent.Type.SynchronismUpdateFailed,
                this.getDocumentToSave()
              )
            );
          let t = GCore.GLocale.get(new GCore.GLocaleKey('GFilesPanel', 'text.error-saving-file'));
        ...
        GSystemDialog.default.alert(
          GCore.GLocale.get(
            new GCore.GLocaleKey('GFilesPanel', 'text.please-inform-valid-file-name')
          ),
          () => {
            (this.view.toggleLoading(false),
              this.view.focusFileNameInput({
                name: GCore.GLocale.get(new GCore.GLocaleKey('GFilesPanel', 'text.untitled')),
              }));
          }
        );
      return this;
    ...
    async openFile(e) {
      ...
      if (await this.drive.canAccessFile(e).catch((e) => false))
        try {
          ...
          (console.log(e.stack), GSystemDialog.default.alert(e.message), this.updateFilesList());
        }
      else
        GSystemDialog.default.alert(
          GCore.GLocale.get(
            new GCore.GLocaleKey('GFilesPanel', 'text.file-can-not-be-accessed-missing-permissions')
          )
        );
    }

    handleFolderClick(e, t) {
      return (
    ...
    _getFullPathNames(e) {
      ...
      for (
        ;
        e.parent &&
        ((GCore = t[e.parent]), GCore) &&
        ((_interopRequireDefault = GCore.folder), _interopRequireDefault.name);
      )
        (n.unshift(_interopRequireDefault.name), (e = _interopRequireDefault));
      return n;
    }

    addFile(e, t, n, _interopRequireDefault) {
    ...
    _triggerFileDownload(e, t, n, _interopRequireDefault, GCore) {
    ...
    _repackNativeBlob(e) {
      ...
      return new Promise(async (n, _interopRequireDefault) => {
        ...
        ((DataModule_1546.onload = async function () {
          ...
          ((module.cancel = (0, CollaborationMergeUtils.chaining)(
            module.cancel,
    ...
    async downloadSelectedFiles() {
      let { ext: exports, type: module } =
          arguments.length > 0 && undefined !== arguments[0] ? arguments[0] : TYPES.DEFAULT_TYPE,
    ...
    async _triggerSelectedFilesDownload(e, t, n, _interopRequireDefault, DataModule_1546) {
      ...
      (d || (d = gDesigner.newInfiniteDocument()),
        ...
        TYPES.text(
          GCore.GLocale.get(new GCore.GLocaleKey('GFilesPanel', 'text.downloading-files')),
          true
        ),
          ...
          GSystemDialog.default.alert(
            $('<div/>')
              ...
              .append(
                $('<span/>').text(
                  GCore.GLocale.get(
                    new GCore.GLocaleKey('GFilesPanel', 'text.error-download-multiple-files')
                  )
                )
              )
              .append(
    ...
    _clearFileFiltersInCaseAvailableFiltersDoesNotMatch(e, t) {
      GCore.GUtil.equals(e, t, true) || this.clearAllFileTypesFromSelectedFilter();
    }

    _navigateDriveToUserFolderOrRoot(e) {
      (this._isUserAllowedToOpenTheFolder(e) || (e = this.drive.getRootFolder()),
        this.drive.setCurrentFolder(e));
    }

    _isUserAllowedToOpenTheFolder(e) {
      return true;
    }
    ...
    async _displayRecentFiles() {
      ...
      if (this.isRootFolder() && !this._showExampleFiles)
        try {
          ...
          throw new Error(
            GCore.GLocale.get(new GCore.GLocaleKey('GFilesPanel', 'text.error-fetching-files'))
          );
        } finally {
        ...
        try {
          ...
          if (l) {
            if (n.hasMoreItemsToLoad()) {
              ...
              ((CollaborationMergeUtils = (GFileDownloadUtils =
                n._sortFilesByMimeType(GFileDownloadUtils)).length),
            ...
            if (n.hasMoreItemsToLoad() && !n._showExampleFiles) {
              try {
                ((await n.drive.getPreviousSelectedFolder()) || (DataModule_1546 = false),
                  ...
                  n.isSaveMode() ||
                    n._isDashboard ||
                    (GFileDownloadUtils = GFileDownloadUtils.concat(
                      await n.drive.loadExampleFiles()
                    )),
    ...
    _getSharedWithMeFolder() {
      return null;
    }

    _updateCurrentFileLoad(e) {
      let module;
      ((module = e > 0 ? (e < this.getQueryLimit() ? -1 : this.CURRENT_FILE_LOAD + e) : -1),
        (this.CURRENT_FILE_LOAD = module));
    }

    getCloudSettingsById(e) {
    ...
    updateCloudItemForUserPermission(e) {
      return gDesigner.getApplicationManager().isOnlyFileOpenFromCloudEnabled() &&
        e &&
    ...
    getCurrentDriveSettings() {
      return this.getCloudSettingsById(this.getCurrentDriveId());
    }

    async setCloudDrive(e) {
      let module = !(arguments.length > 1 && undefined !== arguments[1]) || arguments[1];
      if (!e)
        throw new Error(
          GCore.GLocale.get(
            new GCore.GLocaleKey('GFilesPanel', 'text.error-incorrect-cloud-drive-settings')
          )
        );
    ...
    async getCreateCloudAccountOptions() {
      ...
      if (
        ...
        if (this.SELECTION.length)
          for (var module = 0; module < this.SELECTION.length; module++)
            this.SELECTION[module].getId() === e.getId() && this.SELECTION.splice(module, 1);
    ...
    removeFromSelection(e) {
      ...
      (t > -1 && this.SELECTION.splice(t, 1),
        0 === this.SELECTION.length && this._newClipBoard && (this._newClipBoard = false));
    }
      ...
      this.isClipboardModeCopy(e)
        ? (gDesigner.stats('filespanel_paste-from-copy_cloud'),
          ...
          this.drive
            ...
            .catch((e) => {
              let t = GCore.GLocale.get(new GCore.GLocaleKey('GFilesPanel', 'text.error-moving'));
      ...
      return (
        ...
        (async function () {
          try {
            ...
            return (
              GSystemDialog.default.alert(
                GCore.GLocale.get(new GCore.GLocaleKey('GFilesPanel', 'text.error-deleting'))
              ),
    ...
    toCardView() {
      (this.view.toCardView(),
        gContainer && gContainer.setProperty('GFilesPanel.isFilesListStyle', false));
    }

    toListView() {
      (this.view.toListView(),
        gContainer && gContainer.setProperty('GFilesPanel.isFilesListStyle', true));
    }
    ...
    addFileTypeToSelectedFilter(e) {
      const module = this.drive.getAvailableFileTypesFilter().find((t) => t.type === e);
    ...
    getAvailableExtensions() {
      for (
        ...
        GCore &&
          GCore.secondary &&
          (module ? (exports = module.type !== GCore.type) : (module = GCore));
      }
      return module
        ? exports
    ...
    fileRequiresSourceDownload(e) {
      return !this.drive.getSupportedFileFormats().some((t) => {
        ...
        return (
          t.type === n ||
          (_interopRequireDefault && t.ext.toLowerCase() === _interopRequireDefault.toLowerCase())
        );
      });
    }
    ...
    _isContextMenuAvailableForFile(e) {
      return true;
    }

    logStatsForCurrentFilesSelection(e, t, n) {
    ...
    getFooterSaveDescriptionForFileExtension(e) {
    ...
    manageOpenFolder(e, t, n) {
      this.view.manageOpenFolder(e, t, n);
    }

    static IMAGES_WAIT_TIMEOUT = 2e4;

    static DriveAccountsSettingName = 'designer.filespanel.cloud-accounts';

    static DriveAccountsActiveSettingsName = 'designer.filespanel.cloud-accounts.active';
    ...
    static isMaximized = () => gContainer.getProperty('GFilesPanel.maximized');

    static isFilesGridListStyle = () => gContainer.getProperty('GFilesPanel.isFilesListStyle');
