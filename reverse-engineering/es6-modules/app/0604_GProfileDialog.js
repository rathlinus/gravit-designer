  ...
  class c extends GCore.GObject {
    ...
    async _init(e) {
      ...
      ((this._avatar = $('<div></div>')
        ...
        .append(
          $('<span></span>')
            ...
            .attr(
              'data-title',
              GCore.GLocale.get(new GCore.GLocaleKey('GProfileDialog', 'text.avatar-tooltip'))
            )
      ...
      const h = (e, n, GCore, GPurchasePanel) => {
          let a = $('<div></div>').addClass('tab-panel').append(GCore).appendTo(this._panels);
          $('<li></li>')
            ...
            .on('click', function (n) {
              let GCore = arguments.length > 1 && undefined !== arguments[1] && arguments[1],
      ...
      if (
        (f(c.Tabs.Account) &&
          h(
            GCore.GLocale.get(new GCore.GLocaleKey('GProfileDialog', 'text.details')),
            'gravit-icon-account',
            new a(this._user, this._messageHandler.bind(this), this, require).getHTMLElement(),
            c.Tabs.Account
          ),
        f(c.Tabs.ChangePassword, false) &&
          h(
            GCore.GLocale.get(new GCore.GLocaleKey('GProfileDialog', 'text.change-password')),
        ...
        (await l.hasPurchases({ issued: 'true' }).catch(() => false)) &&
          h(
            GCore.GLocale.get(new GCore.GLocaleKey('GProfileDialog', 'text.purchases')),
    ...
    _updateAvatar() {
      ...
      function module(t) {
        ...
        let require = GCore.GLocale.get(new GCore.GLocaleKey('GAccountPanel', 'text.error'));
        (t && t.message
          ? (require = t.message)
              ...
              if (GPurchasePanel.byteLength > 102400)
                return void module.call(this, {
                  message: GCore.GLocale.get(
                    new GCore.GLocaleKey('GProfileDialog', 'text.avatar-size-too-big')
                  ),
                });
                ...
                gDesigner
                  ...
                  .then((t) => {
                    (exports.removeClass('g-loading'),
                      exports
                        .find('.picture')
                        .css('background-image', 'url('.concat(t.avatar, ')')),
                      exports.find('.picture').removeClass('g-profile-top-avatar'),
                      exports.find('.picture').css({ 'background-color': 'transparent' }),
    ...
    _updateUI(e) {
      this._avatar &&
        ...
        e.hasOwnPictureAvatar()
          ? this._avatar.append(
              $('<div/>').addClass('picture').css('background-image', 'url('.concat(e.avatar, ')'))
            )
          : this._avatar.append(
              $('<div/>')
                .addClass('picture')
