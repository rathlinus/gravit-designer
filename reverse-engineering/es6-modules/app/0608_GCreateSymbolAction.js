...
function (exports, module, require) {
  ...
  class c extends GElementAction {
    ...
    isPro() {
      if (!gDesigner.isEnabledProFeatures()) {
        ...
        if (e) {
          ...
          if (t && t.length) return t.filter((e) => e instanceof GCore.GSymbol).length > 1;
        }
      }
      return false;
    }
        ...
        if (module && module.length)
          for (var require = 0, GEditor = new GCore.GSymbol(), a = module.length - 1; a >= 0; --a) {
            ...
            if (
              ...
              if (!(MenuItemBuilder instanceof GCore.GSymbol && MenuItemBuilder.isMaster()))
                return true;
      ...
      if (!this.isPro() || gDesigner.isEnabledProFeatures()) {
        var exports = 1,
          module = GCore.GLocale.get(
            new GCore.GLocaleKey('GCreateSymbolAction', 'createsymbol.defaultname')
          ),
          ...
          if (a)
            ((a.getSymbols() || []).forEach(function (GEditor) {
              ...
              GSystemDialog.prompt(
                GCore.GLocale.get(
                  new GCore.GLocaleKey('GCreateSymbolAction', 'createsymbol.enternewname')
                ),
                (e) => {
                  if (gDesigner.getActiveDocument() && e) {
                    ...
                    try {
                      for (
                        ...
                        if (
                          GElementAction instanceof GCore.GSymbol &&
                          GElementAction.convertToMaster(e)
                        )
    ...
    static ID = 'modify.createsymbol';

    static TITLE = new GCore.GLocaleKey('GCreateSymbolAction', 'title');

    static SHORTCUT = [GEditor.GKey.Constant.F8];
