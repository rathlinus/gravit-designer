      ...
      return (function (e, t) {
        ...
        for (const t in e)
          'default' !== t &&
            ...
            ((GEditor = (GCore = Object.defineProperty) && Object.getOwnPropertyDescriptor(e, t)) &&
  ...
  ((v.prototype._lockEnter = false),
    ...
    (v.prototype.createHeader = function () {
      ...
      this.renderNewFolderButton(t, () => {
        ...
        if (!t || e.filesPanel.drive.isRootFolder()) return e.filesPanel.updateFilesList();
      ...
      ($('<div />')
        ...
        .append(
          $('<input />')
            ...
            .attr(
              'placeholder',
              GCore.GLocale.get(new GCore.GLocaleKey('GFilesPanel', 'text.search-placeholder'))
            )
            .on('keyup', function (t) {
              t.stopPropagation();
              ...
              var GCore = e.panel
                ...
                GEditor = e.panel.find('.header').find('.search-container').find('.search-icon');
              _interopRequireDefault.length
                ? (GCore.show(), GEditor.hide())
                : (GCore.hide(), GEditor.show());
            })
            .on('focusin', function () {
              e.panel.find('.header').find('.search-container').toggleClass('g-active', true);
            })
            .on('focusout', function () {
              e.panel.find('.header').find('.search-container').toggleClass('g-active', false);
            })
            ...
            .on('keydown', function (t) {
              if (GEditor.GKey.translateCode(t.key) === GEditor.GKey.Constant.TAB) {
                var n = $(this).val().trim();
                (e.filesPanel.search(n), t.preventDefault(), t.stopPropagation());
              }
            })
        )
        .append(
          $('<div />')
            ...
            .on('click', function (t) {
              ...
              var _interopRequireDefault = e.panel
                  ...
                  .find('.search-container')
                  .find('.search-field'),
                GCore = e.panel.find('.header').find('.search-container').find('.search-icon');
          ...
          $('<div />')
            ...
            .on('click', function (t) {
              (t.stopPropagation(),
                ...
                e.panel.find('.header').find('.search-container').find('.search-field').focus());
            })
        )
        .appendTo(t),
    ...
    (v.prototype.addSearchBar = function (e) {
      $('<div />')
        ...
        .hide()
        .text(GCore.GLocale.get(new GCore.GLocaleKey('GFilesPanel', 'text.search-not-found')))
        .appendTo(e);
    }),
        ...
        var _interopRequireDefault = $('<input/>')
          ...
          .val(
            e
              ...
              : GCore.GLocale.get(new GCore.GLocaleKey('GFilesPanel', 'text.untitled'))
          )
          .appendTo(GEditor);
          ...
          gDesigner.getApplicationManager().isSavingToCloudEnabled() &&
            ...
            $('<div/>')
              ...
              .append(
                $('<span/>')
                  .addClass('label')
                  .text(GCore.GLocale.get(new GCore.GLocaleKey('GSaveAction', 'title')))
              )
              .on(
                'click',
                function (e) {
    ...
    (v.prototype._getSelectedExtension = function () {
      const exports = this.panel.find('.save-form-container').find('option:selected').val();
      if (!exports)
        return (
          (this.filesPanel.getDefaultSaveFormat() && this.filesPanel.getDefaultSaveFormat().ext) ||
          u.DEFAULT_TYPE.ext
        );
      const module = this.filesPanel.getPossibleExtensions();
      return (
        (0, CollaborationMergeUtils.getExtensionFromString)(exports, module) || u.DEFAULT_TYPE.ext
      );
    }),
    ...
    (v.prototype._setSelectedExtension = function (e) {
      this.panel
        .find('select.file-extension > option[value="'.concat(e.toUpperCase(), '"]'))
        .prop('selected', true);
    }),
    ...
    (v.prototype.addFile = function (e) {
      ...
      var u = $('<div/>')
        ...
        .on('dragstart', function (t) {
          if (GEditor._isDuringRenaming()) return (t.stopPropagation(), void t.preventDefault());
        ...
        .on('drag', function (e) {
          if (GEditor._isDuringRenaming()) return (e.stopPropagation(), void e.preventDefault());
        ...
        .on(
          'dblclick',
          function (t) {
            if ((t.stopPropagation(), t.preventDefault(), !GEditor._isDuringRenaming()))
          ...
          function (n) {
            (n.stopPropagation(),
              ...
              GEditor._isDuringRenaming() ||
                (this.filesPanel.handleFileClick(e, u), this._updateFileInfoPanel(e, u, module)));
          }.bind(this)
        )
        .on(
          'contextmenu',
      ...
      (e instanceof GCloudStorageItem && (C = e.getPreviewURL()),
        C || (C = e.url_t || e.url_s),
        C ? v.css('background-image', 'url("'.concat(C, '")')) : v.addClass('default-preview'));
      ...
      return (
        ...
        e.hasPermission(GCloudStorageItem.Permission.Open) &&
          ...
          $('<button />')
            ...
            .text(GCore.GLocale.get(new GCore.GLocaleKey('GFilesPanel', 'action.open-design')))
            .on('click', (t) => {
              (t.stopPropagation(),
                e.example || gDesigner.stats('filespanel-view_open_cloudfile'),
    ...
    (v.prototype.updateControls = function (e) {
      ...
      (this.getSearchValue()
        ? (require.addClass('g-disabled'),
          ...
          this.panel.find('.g-files-list').find('.g-gravit-file').length < 1 &&
            this._showNoResultsInfo())
    ...
    (v.prototype._renameFile = function (e, t, n) {
        ...
        try {
          ...
          if (GEditor.trim() && GEditor.trim() !== e.name) {
            ...
            try {
              ...
              (_interopRequireDefault || (_interopRequireDefault = GEditor),
                await GCloudFileBrowser.filesPanel.renameItem(e, _interopRequireDefault));
            } catch (e) {
              return (
                GCloudFileBrowser.toggleLoading(false),
                e && e.badName
            ...
            (e.getType() === GCloudStorageItem.Type.Folder
              ...
              GCloudFileBrowser._fileInfoPanelIsOpen &&
                GCloudFileBrowser._updateFileInfoPanel(e, t, false),
      ...
      DataModule_1163.on('focusout', function () {
        ...
        (GEditor.GKey.translateKey(n) !== GEditor.GKey.Constant.ENTER ||
          u ||
          (gDesigner.stats('filespanel-view_update_cloudfile-name'), h(), (u = true)),
    ...
    (v.prototype._updateMenuItemCaption = function (e, t) {
      this.filesPanel.getSelection().length > 1
        ? e.setCaption(
            GCore.GLocale.get(new GCore.GLocaleKey('GFilesPanel', 'text.selection-multiple'))
              .replace('%selection', t)
              .replace(
                '%multiple',
                GCore.GLocale.get(new GCore.GLocaleKey('GFilesPanel', 'text.multiple'))
              )
          )
        : e.setCaption(t);
    }),
    ...
    (v.prototype._getNativeMenuItems = function () {
      return [
        {
          caption: GCore.GLocale.get(new GCore.GLocaleKey('GFilesPanel', 'action.open')),
          click: () => {
        ...
        {
          caption: GCore.GLocale.get(new GCore.GLocaleKey('GFilesPanel', 'action.rename')),
          click: () => {
            var e = this.filesPanel.getSelection();
        ...
        },
        {
          ...
          update: (e) => {
            var t = this.filesPanel.getSelection();
            const require =
              this.filesPanel.isClipboardModeCut() || this.filesPanel.isClipboardModeCopy();
        ...
        {
          caption: GCore.GLocale.get(new GCore.GLocaleKey('GFilesPanel', 'action.copy')),
          click: () => {
            this.filesPanel.performCopyPaste(p.GFilesPanelClipboardModes.DEFAULT);
          },
          update: (e) => {
            var t = this.filesPanel.getSelection();
        ...
        {
          caption: GCore.GLocale.get(new GCore.GLocaleKey('GFilesPanel', 'action.cut')),
          click: () => {
            this.filesPanel.performCutPaste(p.GFilesPanelClipboardModes.DEFAULT);
          },
          update: (e) => {
            var t = this.filesPanel.getSelection();
            (this._updateMenuItemCaption(
        ...
        {
          caption: GCore.GLocale.get(new GCore.GLocaleKey('GUnshareWithMeAction', 'title')),
          click: async () => {
            if (gDesigner.getShareManager().isShareProRestricted())
              ...
              const t = this.filesPanel.getSelection()[0].getId(),
                n = e.getUID();
              (console.log('About to call unshare with item id: %s and user id: %s', t, n),
    ...
    (v.prototype._createContextMenuDownloadMenu = async function () {
      this._contextMenu
        .createAddItem(
          GCore.GLocale.get(new GCore.GLocaleKey('GFilesPanel', 'action.download-title')),
          () => {
