...
function (exports, module, require) {
  ...
  class r extends GCore.GObject {
    constructor(e, t, n, _interopRequireDefault) {
      ...
      $('<span/>')
      .css('display', 'block')
      .html(GCore.GLocale.get(new GCore.GLocaleKey('GNewFilePrompt', 'text.name-document')))
      )
      .append(
      $('<input/>')
      .attr('type', 'text')
      ...
      r && r.fn && !r.fn(t)
      ? GSystemDialog.default.alert(r.errorMessage.replace('%fileName%', '"'.concat(t, '"')))
      : (e(t), s.close());
      }
      };
      ...
      .concat(
      GCore.GLocale.get(new GCore.GLocaleKey('GNewFilePrompt', 'action.create')),
      '</button>'
      )
      ).on('click', function () {
      ...
      $(
      '<button>' + GCore.GLocale.get(new GCore.GLocaleKey('GLocale', 'cancel')) + '</button>'
      ).on('click', function () {
