      ...
      var p = (s.SHA3 = a.extend({
          ...
          for (var s = 0; s < 24; s++) {
            ...
            for (p = 0; p < 5; p++) {
              var y = u[(p + 4) % 5],
                ...
                b = v.low;
              for (
                g = y.high ^ ((_ << 1) | (b >>> 31)), h = y.low ^ ((b << 1) | (_ >>> 31)), f = 0;
                f < 5;
                f++
              )
            ...
            for (A.high = T.high, A.low = T.low, p = 0; p < 5; p++)
              for (f = 0; f < 5; f++) {
                var G = n[(w = p + 5 * f)],
                  ...
                  D = u[((p + 1) % 5) + 5 * f],
                  L = u[((p + 2) % 5) + 5 * f];
                ((G.high = P.high ^ (~D.high & L.high)), (G.low = P.low ^ (~D.low & L.low)));
              }
            G = n[0];
            var I = d[s];
        ...
        _doFinalize: function () {
          ...
          ((n[CryptoJSCore >>> 5] |= 1 << (24 - (CryptoJSCore % 32))),
            ...
            this._process());
          for (
            var r = this._state, s = this.cfg.outputLength / 8, l = s / 8, c = [], d = 0;
            d < l;
            d++
          ) {
            var u = r[d],
        ...
        clone: function () {
          for (
            var e = a.clone.call(this), t = (e._state = this._state.slice(0)), n = 0;
            n < 25;
            n++
          )
            t[n] = t[n].clone();
