...
function (exports, module, require) {
  ...
  class p {
    ...
    async authenticate() {
      let exports =
        arguments.length > 0 && undefined !== arguments[0] ? arguments[0] : [l.MS_TEAMS_COMMAND];
    ...
    isAuthenticated() {
      ...
      return !(!exports || !this.isTokenValid(exports)) || ((this._authenticated = false), false);
    }

    async _authenticateCommand(e) {
      const module = this,
          ...
          .then((GSystemDialog) =>
            GSystemDialog.authenticate({
              ...
              successCallback: async function (e) {
                ...
                (await require.setTokens(Object.assign(module, GCore)),
              ...
              failureCallback: function (t) {
                t === p.FAIL_REASONS.POPUP_WINDOW_BLOCKED
                  ? require._handleError(
                      ...
                      GCore.GLocale.get(
                        new GCore.GLocaleKey('GMSTeamsAuthenticator', 'text.authenticate')
                      ),
                      (t) => {
                  ...
                  : t === p.FAIL_REASONS.CANCELLED_BY_USER
                    ? (require._handleError(
                        ...
                        GCore.GLocale.get(
                          new GCore.GLocaleKey('GMSTeamsAuthenticator', 'text.try-again')
                        ),
    ...
    async getOrFetchMSTeamsAccessToken() {
      const exports = (this._tokens && this._tokens[l.MS_TEAMS_COMMAND]) || null;
      return (
    ...
    _handleError(e, t, n) {
      ...
      switch (e) {
        ...
        case p.Error.ONEDRIVE_BUSINESS_ERROR:
          _interopRequireDefault = GCore.GLocale.get(
            new GCore.GLocaleKey('GMSTeamsAuthenticator', 'text.onedrive-business-error')
          );
          break;
        ...
        case p.Error.SHAREPOINT_ONEDRIVE_BUSINESS_ERROR:
          _interopRequireDefault = GCore.GLocale.get(
            new GCore.GLocaleKey('GMSTeamsAuthenticator', 'text.sharepoint-onedrive-business-error')
          );
          break;
        ...
        case p.Error.FAILED_TO_OPEN_WINDOW:
          _interopRequireDefault = GCore.GLocale.get(
            new GCore.GLocaleKey('GMSTeamsAuthenticator', 'text.failed-to-open-window')
          );
          break;
        case p.Error.CANCELLED_BY_USER:
          _interopRequireDefault = GCore.GLocale.get(
            new GCore.GLocaleKey('GMSTeamsAuthenticator', 'text.cancelled-by-user')
          );
          break;
        default:
          _interopRequireDefault =
            e && e.message
              ? e.message
              : GCore.GLocale.get(new GCore.GLocaleKey('GMSTeamsAuthenticator', 'text.unknown'));
      }
      GSystemDialog.default.splashScreenError(_interopRequireDefault, t, n);
    }

    async _validateAuthenticatedUser() {
    ...
    async _authWithCorelCloud(e) {
      ...
      if (!module || !module.ok)
        return (this._handleError(p.Error.NOT_REGISTERED), (this._loading = false), false);
    ...
    static CACHED_TOKENS_PROPERTY_NAME = 'msteams.authentication.tokens';

    static _instance = new p();

    static getInstance = () => (p._instance || (p._instance = new p()), p._instance);
