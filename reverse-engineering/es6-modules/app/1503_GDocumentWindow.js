...
function (exports, module, require) {
  ...
  class f extends GCore.GEventTarget {
    ...
    _requiresViewTransformation = true;
    _container = null;
    _document = null;
    _view = null;
    _overlay = null;
    ...
    release() {
      (this._releaseScene(),
        ...
        GFontsProviderManager.getInstance().releaseDocumentListener(this._document.sessionId));
    }
    ...
    centerAndZoom() {
      ...
      if (t)
        if (this._view.getViewConfiguration().multiPageView) {
          ...
          n =
            CollaborationMergeUtils && !CollaborationMergeUtils.isEmpty()
              ? CollaborationMergeUtils
              : t.getPaintBBox();
        } else n = t.getPaintBBox();
      else e.isFixedSized() && (n = new GCore.GRect(0, 0, e.getProperty('w'), e.getProperty('h')));
    ...
    _releaseScene() {
      this._view &&
        ...
        this._view
          .getScene()
          .removeEventListener(GCore.GNode.AfterFlagChangeEvent, this._afterFlagChangeEvent, this));
    }

    activateProgress(e, t) {
      return this._setOverlayContent(
        $('<div></div>')
          ...
          .append(
            $('<div></div>').append(
              $('<button></button>').addClass('cancel-loading').css('display', 'none')
            )
          ),
        t
      );
    ...
    activateCancelLoading(e) {
      var t = this;
      this._overlay
        .find('.cancel-loading')
        .text(GCore.GLocale.get(new GCore.GLocaleKey('GCommonNames', 'text.cancel-loading')))
        .on('click', function () {
    ...
    _documentStatusChanged(e) {
      switch (e.status) {
        ...
        case l.Syncing:
        case l.Downloading:
          var module = GCore.GLocale.get(new GCore.GLocaleKey('GCommonNames', 'text.loading')),
            require = GCore.GLocale.get(new GCore.GLocaleKey('GCommonNames', 'text.loading-file'));
          e.status === l.Saving
            ? ((module = GCore.GLocale.get(new GCore.GLocaleKey('GCommonNames', 'text.saving'))),
          ...
          let GDocumentStatusEvent =
            e.data && e.data.filename ? e.data.filename : this._document.getTitle();
          ...
          (this.activateProgress(
            GDocumentEvent,
            e.data && e.data.cancelHandlerHolder ? e.data.cancelHandlerHolder : null
          ),
            (e.data.text = (e, t) =>
          ...
          e.data.progress = function (e) {
            ...
            if (100 === e || (e - GTools >= 0.5 && GCore - GEditor >= 40)) {
              ...
              ((t = l[0]),
                ...
                AppSettings &&
                  (0, CollaborationMergeUtils.isFunction)(AppSettings) &&
                  AppSettings(e));
            }
          }.bind(this);
          break;
        ...
        case l.DownloadFailed:
          ...
          break;
        case l.LoadFailed:
          module = GCore.GLocale.get(new GCore.GLocaleKey('GCommonNames', 'text.loading-failed'));
          (!e.data || !e.data.text || e.data.text instanceof Function || (module = e.data.text),
            this._setOverlayContent($('<span></span>').text(module)));
          break;
        case l.SaveFailed:
          if ((this._document.setErrored(true), this._document.isCollaborativeTextEditing()))
            return;
          this._setOverlayContent(
            $('<span></span>')
                ...
                $('<p></p>')
                  .text(
                    'string' == typeof e.data
                      ? e.data
                      : GCore.GLocale.get(
                          new GCore.GLocaleKey('GCommonNames', 'text.unable-to-save')
                        )
                  )
              ...
              .append(
                $('<button></button>')
                  .text(GCore.GLocale.get(new GCore.GLocaleKey('GCommonNames', 'text.try-again')))
                  .on(
                    'click',
                    function () {
                      this._document &&
      ...
      if (e && !e.isEmpty()) {
        ...
        if (!n.containsRect(GTools)) {
          ...
          GEditor.GPlatform.scheduleFrame(() =>
            t.scrollBy(CollaborationMergeUtils.getX(), CollaborationMergeUtils.getY())
          );
        }
      }
    }

    static VIEW_MARGIN = 10;
