  ...
  class h extends GAction.default {
    ...
    execute() {
      ...
      if (((this._scene = this._getScene()), this._scene))
        if (
          ...
          const e = require(881) /* module_881 */
    ...
    changeActivePage(e, t) {
      const require = gDesigner
        ...
        .getNextPage(e);
      require
        ? e.setActivePage(require)
        : t === GChangeActivePageAction.default.Type.Next && ScreenfulJS.default.exit();
    }
    _getScene() {
    ...
    _fullScreenRequestDeniedHandler(e) {
      'Fullscreen request denied' === e.reason.message &&
        ...
        GSystemDialog.default.custom({
          title: GCore.GLocale.getValue('GCommonNames', 'text.something-wrong.try-again'),
    ...
    _enterPlayMode() {
      (this._setIsInPlayMode(true),
        this._widget.resize(this._overlay.outerWidth(), this._overlay.outerHeight()));
    ...
    _createExitFullScreenHint() {
      return $('<div/>')
        ...
        .append(
          $('<div/>').html(
            GCore.GLocale.getValue('GPlayAction', 'text.exit-full-screen').replace('%key', () =>
              $('<span/>')
                .addClass('highlight')
