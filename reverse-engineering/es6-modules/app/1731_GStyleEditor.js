...
function (exports, module, require) {
  ...
  function d(e, t, n, CollaborationMergeUtils) {
    ...
    t ||
      ((n = new GCore.GStyle()).setProperty(
        'name',
        GCore.GLocale.get(new GCore.GLocaleKey('GDesignerStyleEditor', 'text.new-style'))
      ),
    ...
    var l = t
        ? GCore.GLocale.get(new GCore.GLocaleKey('GDesignerStyleEditor', 'text.style-editor'))
        : GCore.GLocale.get(new GCore.GLocaleKey('GDesignerStyleEditor', 'text.style-creator')),
          ...
          $('<div/>')
            .append(
              $('<input/>')
                ...
                .prop(
                  'checked',
                  !t || $.inArray(GCore.GStylable.PropertySet.Style, n.getProperty('ps')) >= 0
                )
            )
            .append(
              $('<span/>').text(
                GCore.GLocale.get(new GCore.GLocaleKey('GDesignerStyleEditor', 'text.style'))
              )
            )
            .addClass('checkbox')
        )
            ...
            .append(
              $('<input/>')
                ...
                .prop(
                  'checked',
                  !t ||
                    $.inArray(GCore.GStylable.PropertySet.FillPaintLayers, n.getProperty('ps')) >= 0
                )
            )
            .append(
              $('<span/>').text(
                GCore.GLocale.get(new GCore.GLocaleKey('GDesignerStyleEditor', 'text.fill'))
              )
            )
            .addClass('checkbox')
        )
            ...
            .append(
              $('<input/>')
                ...
                .prop(
                  'checked',
                  !t ||
                    $.inArray(GCore.GStylable.PropertySet.BorderPaintLayers, n.getProperty('ps')) >=
                      0
                )
            )
            .append(
              $('<span/>').text(
                GCore.GLocale.get(new GCore.GLocaleKey('GDesignerStyleEditor', 'text.border'))
              )
            )
            .addClass('checkbox')
        )
          ...
          $('<div/>')
            .append(
              $('<input/>')
                ...
                .prop(
                  'checked',
                  !t || $.inArray(GCore.GStylable.PropertySet.Effects, n.getProperty('ps')) >= 0
                )
            )
            .append(
              $('<span/>').text(
                GCore.GLocale.get(new GCore.GLocaleKey('GDesignerStyleEditor', 'text.effects'))
              )
            )
            .addClass('checkbox')
        )
            ...
            .append(
              $('<input/>')
                ...
                .prop(
                  'checked',
                  t
                    ? $.inArray(GCore.GStylable.PropertySet.Text, n.getProperty('ps')) >= 0
                    : s instanceof GCore.GText
                )
            )
            .append(
              $('<span/>').text(
                GCore.GLocale.get(new GCore.GLocaleKey('GDesignerStyleEditor', 'text.text'))
              )
            )
            .addClass('checkbox')
        ),
      ...
      y = $('<div/>')
        .append(
          $('<button/>')
            .html(
              t
                ? GCore.GLocale.get(new GCore.GLocaleKey('GCommonNames', 'action.apply'))
                : GCore.GLocale.get(new GCore.GLocaleKey('GDesignerStyleEditor', 'action.create'))
            )
            .on('click', function () {
  ...
  function p(e, t) {
    if ((e.empty(), gDesigner.getActiveDocument().getScene().getStyles())) {
      ...
      t ||
        ($('.g-style-editor').find('.no-style').empty(), f($('.g-style-editor').find('.no-style')));
  ...
  function g(e) {
    ...
    return !!(t && t.length > 0) && t[0].getProperty('sref') === e.getReferenceId();
  }
    ...
    ($('<div/>').addClass('style-selector').appendTo(f),
      ...
      n ||
        f
          ...
          .on('click', function () {
            var t = $(this);
            (gDesigner.stats('designerstyle_click_assign', e && e.getReferenceId()),
      ...
      $('<div/>')
        ...
        .append(
          $('<button/>')
            ...
            .on('click', function () {
              (gDesigner.stats('designerstyle_click_disconnect', e && e.getReferenceId()),
                e.disconnectStyle(),
                gDesigner.getActiveDocument().getScene().getStyles().removeChild(e),
                f.remove());
            })
        )
        .append(
  ...
  function f(e, t) {
    ...
    if (n && n.length > 0) {
      ...
      ($('<div/>').addClass('style-selector').appendTo(CollaborationMergeUtils),
        CollaborationMergeUtils.attr('data-selected', r ? 'yes' : 'no')
        ...
        $('<span/>')
          .css('align-self', 'center')
          .text(GCore.GLocale.get(new GCore.GLocaleKey('GDesignerStyleEditor', 'text.no-style')))
          .css('width', '165px')
          .css('margin-left', '10px')
        ...
        var e = this,
          ...
          n = function () {
            var n = $('<div/>'),
              ...
              a = $('<div></div>').addClass('g-style-editor').css('width', '250px');
            if ((f(n, a), gDesigner.getActiveDocument().getScene().getStyles()))
              for (
                var r = gDesigner.getActiveDocument().getScene().getStyles().getFirstChild();
                null !== r;
                r = r.getNext()
              )
                ...
                $('<div/>')
                  .append(
                    $('<button/>')
                      ...
                      .html(
                        GCore.GLocale.get(
                          new GCore.GLocaleKey('GDesignerStyleEditor', 'action.create-new-style')
                        )
                      )
                  )
                  .on('click', function () {
                ...
                $('<div/>')
                  .append(
                    $('<button/>')
                      ...
                      .html(
                        GCore.GLocale.get(
                          new GCore.GLocaleKey('GDesignerStyleEditor', 'action.organize-styles')
                        ) + '...'
                      )
                  )
                  .on('click', function () {
                          ...
                          l = $('<div/>')
                            .append(
                              $('<button/>')
                                .html(
                                  GCore.GLocale.get(
                                    new GCore.GLocaleKey('GCommonNames', 'text.finish')
                                  )
                                )
                                .on('click', function () {
