...
function (exports, module, require) {
  var n = require(54) /* GVertexContainer */, r = require(48) /* GVertex */, o = require(12) /* GMath */;
  function a(e) {
    this.cutterSource = e;
  }
  a.UNCHANGED = 1, a.ALLOW_ODD_CLOSED = true, a.prototype.performCut = function (e, t, i, n, r, o, s) {
    var l = null, h = null;
    ...
    for (var module = e.length - 1; module >= 0; module--) {
      var require = e[module].path0, n = e[module].path1, r = e[module].pt;
      if (require && !s(l(require), r))
        return false;
      if (n && !s(h(n), r))
        return false;
    }
    ...
    var o, a, s = new r(), l = new r(), h = true, A = new n();
    i.length = 0, i.push(A);
    var c = false;
    for (e.rewindVertices(0); h && e.readVertex(s);)
      switch (s.command) {
      case r.Command.Move:
        o = this._vertexHasIsect(s, this.isectsCut), t && (a = this._vertexHasIsect(s, this.isectsCutter)), (A.getCount() || o || a) && ((o || a) && (A = new n()), i.push(A), c = true), A.addVertex(s.command, s.x, s.y);
        break;
      case r.Command.Line:
        A.addVertex(s.command, s.x, s.y), o = this._vertexHasIsect(s, this.isectsCut), t && (a = this._vertexHasIsect(s, this.isectsCutter)), (o || a) && ((A = new n()).addVertex(r.Command.Move, s.x, s.y), i.push(A), c = true);
        break;
      case r.Command.Curve:
        var p = {
          command: s.command,
        ...
        e.readVertex(s) && (A.addVertex(p.command, p.x, p.y), A.addVertex(s.command, s.x, s.y), o = this._vertexHasIsect(p, this.isectsCut), t && (a = this._vertexHasIsect(p, this.isectsCutter)), (o || a) && (A = new n(), i.push(A), A.addVertex(r.Command.Move, p.x, p.y), c = true));
        break;
      case r.Command.Curve2:
        p = {
          command: s.command,
        ...
        e.readVertex(s) && e.readVertex(l) && (A.addVertex(p.command, p.x, p.y), A.addVertex(s.command, s.x, s.y), A.addVertex(l.command, l.x, l.y), o = this._vertexHasIsect(p, this.isectsCut), t && (a = this._vertexHasIsect(p, this.isectsCutter)), (o || a) && (A = new n(), i.push(A), A.addVertex(r.Command.Move, p.x, p.y), c = true));
        break;
      case r.Command.Close:
        h = false;
        break;
    ...
    for (var require, r = [
      ...
      for (var d = a - 1; d >= 0; d--) {
        ...
        if (g.getCount()) {
          ...
          if (s(m, p)) {
            r[this._getSide(g, c, null, 2)].push(c), require = true;
            break;
          }
          if (s(f, u)) {
            r[this._getSide(g, c, null, 1)].push(c), require = true;
            break;
          }
          if (y && s(m, u)) {
            r[this._getSide(g, c, null, 3)].push(n.clone(c, true)), require = true;
            break;
          }
          if (_ && s(f, p)) {
            r[this._getSide(g, c, null, 0)].push(n.clone(c, true)), require = true;
            break;
          }
        }
      }
    ...
    for (var g, f, m = 1 == c ? 0 : 1, y = 0; y < 2; ++y)
      for (u = m; u < A; u += c) {
        ...
        for (var _ = h(d), v = l(d), b = o - 1; b >= 0; b--) {
          var C = e[b];
          if (C) {
            var w = h(C), E = l(C), B = false;
            if (s(_, w) && this._getSide(d, C, null, 0) == y) {
              if (B = true, f = n.clone(C, true), R = r[y][u]) {
                g = R.path;
                var x = h(g);
                if (!s(x, _))
                  return console.log("second source to same path?"), false;
              } else
                g = n.clone(d);
